#!/usr/bin/env bash
# ★★★★★ THE GUEST VALIDATION LADDER — ported from nvkvm-pv, 2026-09-21 (w823).
#
# `[owner]` *"you may copy test infra from old code and even nvkvm-pv, since a lot doesn't change
# across architecture revisions ... validate.sh etc."*
#
# ## Why this was worth porting rather than writing
#
# 30 checks over a guest's GPU stack, and **only two of them knew anything about nvkvm** — the
# rest ask the DRIVER and the CUDA/Vulkan/EGL runtimes questions that are true of any VMM. The
# probes are self-building heredocs that `dlopen` their libraries, so the only build dependency is
# `cc`.
#
# ## ★★★ The property that makes it worth more than its size: UNTESTED is a verdict
#
# It reports PASS / FAIL / SKIP / **UNTESTED**, and exits **3** for *"CANNOT VALIDATE"*.
# ⊘ That exists because a toolchain-stripped image once reported *"5 PASS, 1 FAIL, 24 SKIP"* while
# **never running CUDA, Vulkan or GL at all** — a green-looking run over an experiment that did not
# happen. ⇒ A skip is not a pass, and an inability to test is not a skip.
#
# ## ⊘ WHAT WAS CHANGED, and nothing else
#
# The two nvkvm-coupled checks were retargeted, not deleted — deleting them would shrink the
# ladder to fit the implementation, which is what a grader must never do:
#   * `guest_module` — asked `lsmod | grep nvkvm_guest`. Under kayfabe the guest runs the
#     **stock, unpatched** NVIDIA driver and there is no guest module of ours at all, so the check
#     now asserts `nvidia` is loaded and records the absence of any kayfabe module as the
#     **expected** state. ⚠ That inversion IS the architectural difference between Mode 1 and
#     Mode 2, so it is stated rather than dropped.
#   * `abi_profile` — greped dmesg for an nvkvm ABI line. Kayfabe prints no such line; the check
#     now reports UNTESTED rather than SKIP, because "we have no instrument" is not "there is
#     nothing to find".
#
#
...
# ⊘ PORTED w823: under kayfabe the guest runs the STOCK driver; there is no module of ours.
MODINFO="$(lsmod 2>/dev/null | awk '$1=="nvidia"{print $1" size="$2" refcnt="$3}')"
if [ -n "$MODINFO" ]; then
    record guest_module PASS "$MODINFO"
else
    LOADED_NV="$(lsmod 2>/dev/null | awk '/nvidia/{print $1}' | tr '\n' ' ')"
    record guest_module FAIL "the stock nvidia module is not loaded (modules seen: ${LOADED_NV:-none})"
fi
