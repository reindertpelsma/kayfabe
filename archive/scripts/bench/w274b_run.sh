...
# ⊘⊘ CORRECTED 2026-08-12 (w275), IN PLACE: the OPERAND-PIN baseline below read `88`, which
#    is the DOORBELL-XLATE value one row up — a number carried forward from w273's SUMMARY
#    instead of read from the log. `[measured, read from the artefact]`
...
#    ⇒ the grader was comparing every future boot's OPERAND-PIN against the wrong constant.
echo "    DOORBELL-XLATE  = $(grep -c 'DOORBELL-XLATE' "$Q" 2>/dev/null)   (w271_pin: 88)"
echo "    OPERAND-PIN     = $(grep -c 'OPERAND-PIN' "$Q" 2>/dev/null)   (w271_pin: 156, w274_pin: 223)"
