...
fn backing_a_framebuffer_leaf_allocates_vidmem_and_places_it_at_the_guests_own_va() {
    ...
    let backed = device
        .back_fb_leaf(GPU, PDB, LEAF_VA, LEAF_LEN, LEAF_PHYS, FbLeafBacking::Vidmem)
...
fn joining_a_framebuffer_leaf_mints_no_vidmem_and_hands_a_backing_to_the_vmm() {
    ...
    let joined = device
        .back_fb_leaf(GPU, PDB, LEAF_VA, LEAF_LEN, LEAF_PHYS, FbLeafBacking::Joined)
...
fn a_leaf_below_the_fixed_map_granule_is_refused_and_not_rounded_up() {
    ...
    let e = device
        .back_fb_leaf(GPU, PDB, LEAF_VA, 0x1000, LEAF_PHYS, FbLeafBacking::Joined)
        .expect_err("refused");
    assert!(
...
fn a_leaf_base_that_is_not_granule_aligned_is_refused() {
    ...
    let e = device
        .back_fb_leaf(GPU, PDB, GpuVa(LEAF_VA.0 + 0x1000), LEAF_LEN, LEAF_PHYS, FbLeafBacking::Joined)
        .expect_err("refused");
    assert!(
...
fn the_system_proc_may_not_back_a_framebuffer_leaf() {
    ...
    let refused = device.with_proc_mut(sys, |p| {
        kayfabe_fwd::plan_back_fb_leaf(p, GPU, PDB, LEAF_VA, LEAF_LEN, LEAF_PHYS, FbLeafBacking::Joined)
