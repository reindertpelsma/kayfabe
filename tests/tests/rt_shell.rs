...
fn run_script(mode: LockMode) -> (Vec<String>, RtSnapshot) {
    ...
    for (i, &pid) in pids.iter().enumerate() {
        ...
        log.push(format!(
            "doorbell-ce[{i}] {:?}",
            device.doorbell(None, gpu_of(i), MockArch::token_for(ce_vchid(i)), &[], None)
        ));
    }
    ...
    log.push(format!(
        ...
        device.doorbell(None, gpu_of(2), MockArch::token_for(gr_vchid(2)), &[], None)
    ));
...
fn spine_ops_acquire_no_proc_lock_via_get_mut() {
    ...
    device
        .doorbell(None, gpu_of(0), MockArch::token_for(gr_vchid(0)), &[], None)
        .expect("doorbell");
    assert_eq!(
        lock::acquisitions(LockRank::Proc) - proc_before,
        2,
                            ...
                            match rng.next() % 100 {
                                ...
                                85..=89 => {
                                    let mut section = BlockingSection::enter(
                                        "rt_shell fuzz — a legal blocking section",
                                    );
                                    section.run(|| ());
                                }
