...
fn two_process_gpu() -> (Gpu, MockVmm, std::sync::Arc<std::sync::Mutex<nvkvm_mocks::RmRecorder>>)
{
    ...
    s.compute_process(nvkvm_arch::ids::HClient(0xAA), A_PDB, identical_handles(0x10, 0x11));
    s.compute_process(nvkvm_arch::ids::HClient(0xBB), B_PDB, identical_handles(0x20, 0x21));
    for ev in s.events {
...
fn t14_identical_va_disjoint_backing() {
    ...
    let vas_allocs: Vec<_> =
        log.log.iter().filter(|(_, v)| matches!(v, RmVerb::AllocVaSpace { .. })).collect();
    assert_eq!(vas_allocs.len(), 2, "one host VAS per proc");
    assert_ne!(vas_allocs[0].0, vas_allocs[1].0, "host VASes on distinct isolates");
}
...
fn t14_doorbell_demux_routes_to_own_isolate() {
    ...
    let log = recorder.lock().unwrap();
    let sched: Vec<_> =
        log.log.iter().filter(|(_, v)| matches!(v, RmVerb::Schedule { .. })).collect();
    assert_eq!(sched.len(), 2, "each proc schedules its own channel (no shared one-shot)");
