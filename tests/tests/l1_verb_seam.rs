...
fn progress_under_pending_verb_intra_proc() {
    for mode in [LockMode::Degenerate, LockMode::Sharded] {
        ...
        let b = thread::spawn(move || {
            ...
            let rung = b_device
                .doorbell(None, GPU, MockArch::token_for(CE), &[], None)
...
fn worker_death_retires_the_proc_loudly_and_never_resurrects() {
    ...
    for mode in [LockMode::Degenerate, LockMode::Sharded] {
        ...
        assert_eq!(
            device
                .doorbell(None, GPU, MockArch::token_for(GR2), &[], None)
...
fn idempotent_replay_emits_no_verbs_and_takes_no_worker() {
    ...
    let pid = pids[0];
    let first = device
        .forward_engine_object(GPU, GR, kayfabe_tests::COMPUTE_CLASS, &[], None)
