/* ★ kayfabe V3_GFX_TESTSET: a VERBATIM copy of nvkvm-pv tests/perf/apps/egl_offscreen.c, plus ONE
 * addition at the end: a whole-framebuffer readback digest (EGLOFF_FB_MD5, FNV-1a 64 of the 1024x1024
 * RGBA image). nvkvm-pv graded this program on "no GL error" alone; the digest lets the test set grade
 * the pixels against bare metal instead (a GL error is not the only way a render can be wrong). */
...
int main(int argc,char**argv){
    ...
    { /* kayfabe: the whole framebuffer, digested (see the header) */
        unsigned char *fb=malloc((size_t)W*W*4); unsigned long long h=1469598103934665603ull;
        glReadPixels(0,0,W,W,GL_RGBA,GL_UNSIGNED_BYTE,fb);
        for(size_t i=0;i<(size_t)W*W*4;i++){h^=fb[i];h*=1099511628211ull;}
        printf("EGLOFF_FB_MD5 %016llx\n",h); free(fb); }
    return 0;
}
