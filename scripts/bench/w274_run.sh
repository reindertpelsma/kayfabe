...
PFX=${W274_TAG_PREFIX:-w274}
OUT=/workspace/${PFX}_run.log
exec >"$OUT" 2>&1
finish() { echo "=== W274 EXIT rc=$1 at $(date -Is) ==="; exit "$1"; }
echo "=== W274 START $(date -Is) pid=$$ ==="
...
export POST_CAPTURE_HOOK=$REPO/scripts/bench/cup2_hook_procspin.sh
export NVDIFF_SRC_DIR=${NVDIFF_SRC_DIR:-/workspace/nvdiff_src}
export NVD_GUEST_OUT=/workspace/bench/nvdiff_guest_ce_r1.jsonl
export GQ_TIMEOUT=900
export BOOT_TIMEOUT=180

for f in scripts/bench/cup2_hook_procspin.sh; do
...
echo "    stdout:"; sed 's/^/      /' /workspace/bench/nvdiff_guest_ce_r1.jsonl.stdout 2>/dev/null | head -20
finish 0
