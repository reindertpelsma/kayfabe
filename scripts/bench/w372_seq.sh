#!/usr/bin/env bash
# ★★★★★ w372 — NAME THE FAILING CALL. Zero rebuild.
#
...
if [ "${W371_ROLE:-}" = hook ]; then
  ...
  echo "=== w372 NAME-THE-FAILING-CALL tag=$TAG (nvidia-smi x8 + per-step guest dmesg) ==="
  ...
  RD="sudo -n dmesg 2>/dev/null || sudo -n cat /dev/kmsg 2>/dev/null || dmesg"
  probe=$($G "sudo -n dmesg 2>&1 | head -1" 2>&1 | tr -d '\r')
  case "$probe" in *"not permitted"*|*"a password is required"*|*"command not found"*)
      echo "⊘ W372_UNMEASURED: the guest cannot read the kernel buffer ('$probe'). NOT an absence of NVRM lines." ;;
  esac
  base=$($G "$RD | wc -l" 2>/dev/null | tr -d '\r'); base=${base:-0}
  ...
  for i in 1 2 3 4 5 6 7 8; do
    ...
    d=$($G "$RD | tail -n +$((base+1)) | grep -aiE 'NVRM|Xid|nvidia' | tail -25" 2>&1 | tr -d '\r')
    if [ -n "$d" ]; then echo "$d" | sed "s/^/  SMI#$i dmesg| /"; else echo "  SMI#$i dmesg| (no new NVRM/Xid lines)"; fi
    base=$($G "$RD | wc -l" 2>/dev/null | tr -d '\r'); base=${base:-$base}
  done
  echo "--- ★ persist the FULL guest dmesg (the harness has never done this) ---"
  $G "$RD" > "/workspace/bench/run_${TAG}_guest_dmesg_AFTER.log" 2>/dev/null
  ...
  echo "=== w372 STEPS DONE ==="
  exit 0
fi
case "${1:-}" in run) ;; *) echo "usage: $0 run" >&2; exit 64 ;; esac
REPO=${KAYFABE_REPO:?}
export KAYFABE_REPO="$REPO" KAYFABE_TAG=${KAYFABE_TAG:-w372seq} W371_ROLE=hook STEP_TIMEOUT
