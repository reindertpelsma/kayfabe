#!/usr/bin/env bash
# ★★★★★ w366 — SEPARATE "SECOND PROCESS" FROM "AFTER nvidia-smi", AND BOUND EVERY STEP.
#
...
if [ "${W362_ROLE:-}" = hook ]; then
  ...
  echo "=== w366 SEQUENTIAL-PROCESS A/A2/B/C/D tag=$TAG step_timeout=${STEP_TIMEOUT}s ==="
...
PYEOF
  ...
  echo "=== w366 STEPS DONE ==="
  exit 0
fi
case "${1:-}" in run) ;; *) echo "usage: $0 run" >&2; exit 64 ;; esac
REPO=${KAYFABE_REPO:?}
export KAYFABE_REPO="$REPO" KAYFABE_TAG=${KAYFABE_TAG:-w366seq} W362_ROLE=hook STEP_TIMEOUT
