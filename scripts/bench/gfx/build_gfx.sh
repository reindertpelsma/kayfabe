...
mkdir -p "$OUT"; rm -f "$OUT"/vk_gfx "$OUT"/egl_gfx "$OUT"/*_spv.h   # ⊘ [ -x ] cannot tell fresh from stale
...
echo "BUILD_GFX_OK $(md5sum "$SRC"/vk_gfx.c "$SRC"/egl_gfx.c "$SRC"/refcheck.h | md5sum | cut -c1-12)"
