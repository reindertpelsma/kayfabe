...
set -uo pipefail
OUT=/workspace/w289_cup2.log
exec >"$OUT" 2>&1
finish() { echo "=== W289 CUP2 EXIT rc=$1 at $(date -Is) ==="; exit "$1"; }
echo "=== W289 CUP2 START $(date -Is) pid=$$ ==="
...
# ⊘ Unanchored has printed `[CUP2_RC=0 CUP2_RC=1]` on two consecutive rungs: it would report
#   the campaign's headline success value on a FAILING arm. Both are printed, always.
# =========================================================================================
echo "=== ★★★★★ CUP2_RC — ANCHORED (baseline 1 at rev aea02a52; 124 before the notifier work) ==="
