#!/usr/bin/env bash
# ★★★★★ w374 — PER-CYCLE guest-kernel delta across the 5th-device-open wall.
#
...
if [ "${W373_ROLE:-}" = hook ]; then
  ...
  echo "=== w374 PER-CYCLE GUEST-KERNEL DELTA tag=$TAG ==="
  ...
  echo "--- ★★★★★ THE PRE-REGISTERED FALSIFIER (fix 147694ff) ---"
  Q=/workspace/bench/run_${TAG}_qemu.log
  echo "GSP-PUBLISH lines (one per bind; same_instance MUST be false on every cycle):"
  grep -a "GSP-PUBLISH" "$Q" 2>/dev/null | sed 's/^/    /' | head -20
  echo "  same_instance=true  count = $(grep -ac "same_instance=true" "$Q" 2>/dev/null || echo 0)   ⇒ MUST be 0"
  echo "  PC-D7 COLLISION    count = $(grep -ac "PC-D7 COLLISION" "$Q" 2>/dev/null || echo 0)   ⇒ >0 proves the address was lying, and that we caught it"
  echo "  distinct gpas      = $(grep -aoE "GSP-PUBLISH gpa=0x[0-9a-f]+" "$Q" 2>/dev/null | sort -u | wc -l) over $(grep -ac "GSP-PUBLISH" "$Q" 2>/dev/null || echo 0) binds"
  echo "     ⚠ a REPEATED gpa with same_instance=false is the smoking gun: the allocator recycled and we did NOT believe it"
  echo "--- ★ and the guest-side symptoms that must be GONE ---"
  for pat in "Bad sequence number" "WPR2 already up" "RmInitAdapter failed"; do
    n=$(cat ${D}.cyc* 2>/dev/null | grep -ac "$pat" || echo 0)
    echo "  \"$pat\" = $n   ⇒ MUST be 0"
  done
  echo "=== w374 STEPS DONE ==="
  exit 0
fi
case "${1:-}" in run) ;; *) echo "usage: $0 run" >&2; exit 64 ;; esac
REPO=${KAYFABE_REPO:?}
export KAYFABE_REPO="$REPO" KAYFABE_TAG=${KAYFABE_TAG:-w374seq} W373_ROLE=hook STEP_TIMEOUT
