...
chmod +x "$T/scripts/bench/w371_seq.sh"
...
STEP_TIMEOUT=600 W329_WORKLOAD="w371_seq.sh run" timeout 3000 bash "$A" "$T" w371seq 1 \
   STEP_TIMEOUT=600 W329_WORKLOAD="w371_seq.sh run" >/dev/null 2>&1
P=/workspace/bench/run_w371seq1_probe.log; Q=/workspace/bench/run_w371seq1_qemu.log
echo "############ GRADED ############"
[ -s "$Q" ] || echo "NO QEMU LOG - THE BOOT NEVER HAPPENED"
sed -n "/=== w371 SEQUENTIAL/,\$p" "$P" 2>/dev/null | head -40
...
H=/workspace/bench/run_w371seq1_hostdmesg.log; [ -f "$H" ] && grep -iE "xid" "$H" | sed -E "s/.*(ENGINE [A-Z0-9_]+).*faulted @ ([0-9a-fx_]+).*type ([A-Z_]+).*/\1 @ \2 \3/" | sort | uniq -c || echo "⊘ NO HOST DMESG FILE — Xid UNMEASURED, not 0"
...
H=/workspace/bench/run_w371seq1_hostdmesg.log
if [ -f "$H" ]; then echo "hostdmesg_lines=$(wc -l < $H) Xid=$(grep -c Xid $H)"; else echo "⊘ NO HOST DMESG FILE — Xid is UNMEASURED, not 0"; fi
echo "--- guest NVRM ---"; grep -ac NVRM /workspace/bench/run_w371seq1_dmesg.log 2>/dev/null
echo "=== DONE $(date -Is) ==="
echo "EXIT=0"
