...
REV=$(git rev-parse 77767724)
cd "$T" || { echo "EXIT=90"; exit 90; }
git fetch -q /root/w364.bundle w337-gpu-name-seam:w364 && git checkout -q $REV || { echo "EXIT=91"; exit 91; }
echo "HEAD=$(git rev-parse HEAD)"
echo "tracked dirt: [$(git status --porcelain --untracked-files=no)]"
chmod +x "$T/scripts/bench/w364_seq.sh"
...
case "$QS" in kayfabe-rev:77767724*) echo "GATE ok $QS";; *) echo "GATE REFUSED ($QS)"; echo "EXIT=93"; exit 93;; esac
export KAYFABE_GPU_NAME="NVIDIA GeForce RTX 3060"
export KAYFABE_GPU_SHORT_NAME="RTX 3060"
STEP_TIMEOUT=600 W329_WORKLOAD="w364_seq.sh run" timeout 3000 bash "$A" "$T" w364seq 1 \
   STEP_TIMEOUT=600 W329_WORKLOAD="w364_seq.sh run" >/dev/null 2>&1
P=/workspace/bench/run_w364seq1_probe.log; Q=/workspace/bench/run_w364seq1_qemu.log
echo "############ GRADED ############"
[ -s "$Q" ] || echo "NO QEMU LOG - THE BOOT NEVER HAPPENED"
sed -n "/=== w364 SEQUENTIAL/,\$p" "$P" 2>/dev/null | head -40
...
H=/workspace/bench/run_w364seq1_hostdmesg.log
if [ -f "$H" ]; then echo "hostdmesg_lines=$(wc -l < $H) Xid=$(grep -c Xid $H)"; else echo "⊘ NO HOST DMESG FILE — Xid is UNMEASURED, not 0"; fi
echo "--- guest NVRM ---"; grep -ac NVRM /workspace/bench/run_w364seq1_dmesg.log 2>/dev/null
echo "=== DONE $(date -Is) ==="
echo "EXIT=0"
