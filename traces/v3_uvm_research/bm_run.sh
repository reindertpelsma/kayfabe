...
# build: nvcc -O2 -arch=sm_86 -cudart static -o holder holder.cu (and the two probes)
...
# fault_stats exists only while a UVM user holds the GPU: keep one alive.
"$D/holder" > /tmp/bm_holder.out 2>&1 & HOLDER=$!; trap "kill $HOLDER 2>/dev/null" EXIT
for _ in $(seq 50); do grep -q HOLD_READY /tmp/bm_holder.out 2>/dev/null && break; sleep 0.2; done
echo "HOLDER pid=$HOLDER $(cat /tmp/bm_holder.out)"
