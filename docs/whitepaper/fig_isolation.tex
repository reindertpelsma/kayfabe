...
\node[box, fill=kfhost!30, draw=kfhostln, minimum width=178mm, minimum height=11mm] at (0,-29)
  {\textbf{\color{kfhostln}What is actually running, as of this document.}\\[2pt]
   \tiny The per-\tc{Proc} plane ownership above is \textbf{built and heavily tested} in Rust — against
   \tc{MockIsolateFactory}, in one process, with no OS sandbox.\\[-0.5pt]
   \tiny \textbf{No real isolate process has ever been spawned by this codebase}: \tc{IsolateFactory} and
   \tc{RmBackend} have exactly one implementation each, and it is the mock.\\[-0.5pt]
   \tiny The sandbox posture described above is the \emph{C artifact's} Mode-1 stub, cited as the target.
   Porting it is L1 work that has not been done.};
