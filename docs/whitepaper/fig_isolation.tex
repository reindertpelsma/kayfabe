...
\node[box, fill=kfhost!30, draw=kfhostln, minimum width=178mm, minimum height=11mm] at (0,-29)
  {\textbf{\color{kfhostln}What is actually running, as of this document (2026-08-20).}\\[2pt]
   ...
   \tiny \textbf{\color{kfhostln}⊘ What is NOT established is the multi-\tc{Proc} axis this figure
   exists to argue}: every graded boot to date runs \textbf{one} guest CUDA process. Bug \#14 —
   the reason this codebase exists —\\[-0.5pt]
   \tiny is structurally addressed and has \emph{never} been shown fixed on a GPU. \textbf{There is
   also no tenant axis at all} — no \tc{VmId} anywhere in \file{crates/}; cross-VM separation is
   incidental to two host processes.};
