...
% and cross-checked against Cargo.lock, at 6ce8599. Rows = the depending
...
% graph is acyclic.
%
% ★ The ordering CHANGED at this revision. `gsp` used to sit at position 5,
% above `abi` and `trace`; it now depends on both, so it had to move below
% them. That re-ordering is the figure's own record of the crate ceasing to be
% a leaf.
% ===========================================================================
...
% -- highlight the gsp row + column ------------------------------------------
\fill[kfcoreln, opacity=0.09] (0,{-8*\cs}) rectangle ({\N*\cs},{-9*\cs});
\fill[kfcoreln, opacity=0.09] ({8*\cs},0) rectangle ({9*\cs},{-\N*\cs});
...
\dep{10}{1}\dep{10}{2}\dep{10}{3}\dep{10}{5}
...
\dep{15}{1}\dep{15}{2}\dep{15}{3}\dep{15}{4}\dep{15}{5}\dep{15}{6}\dev{15}{10}\dep{15}{11}\dep{15}{12}
\dep{16}{1}\dep{16}{11}\dep{16}{13}\dep{16}{15}
\dep{17}{1}\dep{17}{2}\dep{17}{3}\dep{17}{4}\dep{17}{5}\dep{17}{6}\dep{17}{7}\dep{17}{8}
\dep{17}{9}\dep{17}{10}\dep{17}{11}\dep{17}{12}\dep{17}{13}\dep{17}{14}\dep{17}{15}\dep{17}{16}

% -- annotations -------------------------------------------------------------
\draw[ar, kfcoreln] ({8.5*\cs},{-\N*\cs-6}) -- ({8.5*\cs},{-\N*\cs+1});
\node[anchor=north, font=\sffamily\tiny, color=kfcoreln, align=center]
  at ({8.5*\cs},{-\N*\cs-6})
  {\textbf{gsp} now depends on \textbf{abi}\\ and \textbf{trace} — but the only\\
   crate that depends on \textbf{gsp}\\ is \code{tests}. \textbf{No production}\\
   \textbf{crate consumes it.}};
...
\node[anchor=north west, font=\sffamily\scriptsize, align=left]
  ...
  crate $c$. All 72 \code{[dependencies]} edges ({\color{kfcoreln}$\bullet$}) and all
  ...
  \code{trybuild} and \code{proptest}, both dev-only. Thirteen of the sixteen crates
  have \emph{no} external dependency of any kind.\\[3pt]
  ...
  \code{trace} and \code{vmm} \emph{ports}; \code{gsp} (core) depends on the
  \code{abi} and \code{trace} \emph{ports}. "Core" and "port" are not disjoint
