...
125 commits, 45\,421 lines of implementation and 52\,931 lines of test code, developed
2026-07-24 to 2026-07-28; it has never touched a GPU. This document is pinned to Rust HEAD
\code{b612a91} and C HEAD \code{2899a74}.

\smallskip
\textbf{What moved since the previous pin (\code{6ce8599}), and why this is a rewrite of the
status narrative rather than a version bump.} Twelve commits landed in between, and they falsify
this paper's previous headline. That revision said \code{kayfabe-gsp} was a working island — "no
production crate depends on it, \code{RpcCommand} has zero references outside its own crate,
therefore \emph{the critical path is the bridge, not the crate}". \textbf{The bridge now exists}:
a new crate, \code{kayfabe-rmrpc}, built in four stages (\code{76a0077}, \code{2dac655},
\code{712e370}, \code{3a1704f}) against a design document written first
(\file{docs/design/gsp\_core\_bridge.md}, \code{bdd7625}). A decoded GSP RPC now becomes an
\code{RmEvent} and reaches \code{Gpu::apply} inside \emph{production} crates.
