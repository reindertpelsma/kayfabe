...
\node[box, fill=white, draw=kfcoreln, line width=0.8pt,
  ...
  {\textbf{\color{kfcoreln}\tc{kayfabe-gsp} — BUILT, 3\,828 L, 8 modules (S0--S5)}\;$\rightarrow$\;\tc{RpcCommand}\\[1pt]
   ...
   \textbf{\color{kfcoreln}★ \tc{kayfabe-rmrpc} — BUILT, 1\,033 L (B0--B4)}\;$\rightarrow$\;\tc{RmEvent}\\[1pt]
   \tiny \tc{translate} (stateless free fn) $\cdot$ \tc{GraphPolicy}: translate $\rightarrow$ \tc{Gpu::apply}
   $\rightarrow$ reply $\cdot$ 15 named refusals + a bounded census\\[1.5pt]
   \tiny \textbf{\color{kfhostln}No \tc{GuestRam} and no \tc{Device} implementor outside \tc{tests/},
   so nothing feeds it a byte a guest wrote.}};
...
\node[coreb, minimum width=76mm, minimum height=16mm] (ctl) at (-45,37.5)
  {\textbf{C}\; the CONTROL plane — \tc{Gpu::apply(RmEvent)}\\[1pt]
   ...
   \tiny \textbf{\color{kfhostln}★ \tc{sync\_rpc\_mappings} HAS NO PRODUCER HERE —
   \tc{MAP\_MEMORY\_DMA} is \tc{\_STUB}}\\[-0.5pt]
   \tiny \textbf{\color{kfhostln}on every GSP-client part; the real sources are
   \tc{fwd}/\tc{mmu} work and unbuilt.}};
