...
| ⊘⊘⊘ **REVERSED 2026-09-15 — THE VERB IS BACK. Read this before acting on the row.** It was re-issued on **request tags 30/31, reply 15** (25/12 stay retired, never renumbered) under an explicit owner ruling — `bar1_passthrough_device_local_host_visible.md` §4 item 1, *decision (b)*, **granted conditionally** 2026-09-14: the VMM may transiently hold a `/dev/nvidia<N>` descriptor **solely to `mmap` it**, issuing no escape on it and closing it the instant `mmap` returns. Exercised end to end in a boot: `DEVICE_VIEW=OK sentinel_roundtrip=true released=true`. ⚠ **Do not re-apply this row's deletion.** The row below is the 2026-09-12 history and was correct when written — it is kept, not edited, because the reasoning in it is still the reasoning for *why* an orphan gets discarded. | | ✅ **DONE 2026-09-12 (SUPERSEDED)** `export_device_view` + wire tag 25 | `kayfabe-isolate/src/lib.rs:3210` | **Zero senders of any kind**, tests included. The newest verb on the wire and the deadest. ⊘ **One caller the row missed, and it is not on the wire:** `rmladder --bar1-crossing` calls it on a concrete `HostRmBackend`, in-process. The trait verb, the `Worker` wrapper, `DeviceView`, `Request::ExportDeviceView` (tag 25) and `Reply::DeviceView` (tag 12) are deleted; the **host impl survives as an inherent method** on `HostRmBackend` so the bare-metal probe still builds. Tags 25/12 retired in place, never renumbered. |
