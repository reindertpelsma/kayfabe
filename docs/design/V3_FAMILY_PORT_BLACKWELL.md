...
| 6 | `kf-rm hostquery::query_lce_pce_masks` | ask `CE_GET_CE_PCE_MASK` for LCE 0, 1, … until the first refusal | `[measured bws3]` UVM_REGISTER_GPU `0x56` ← `W349REFUSE 0x20802a02 NoMaskForEngine {COPY4, stated 1}`: GB203's floorswept LCE2 ended the list | ask over `CE_GET_ALL_CAPS.present`; a hole is `NO_PCE_MASK`, refused at the serve like a missing row; a contiguous part (GA106 `0x0f`) asks exactly the same 5 |
| 7 | `kf-chan translated` (the Translated rewrite) | a physical CE launch can always be made virtual | `[measured bws3]` **Xid 71 (CE4 error)** on our host ring for every PMA scrub; the guest's `scrubberDestruct` then timed out. RM's Hopper+ **fast scrub** (`LAUNCH_DMA` `MEMORY_SCRUB_ENABLE` 23:23, `clc8b5.h:84`; chosen by `memmgrMemUtilsCheckMemoryFastScrubEnable_GH100` for class ≥ `0xC8B5`, dst phys LOCAL_FB, 4 KiB) *"only works with physical addressing"* (`uvm_hopper_ce.c:185-196`) | a scrub with a physical dst is emitted as the equivalent **virtual byte zero-fill** (remap `DST_X=CONST_A=0`), the guest's remap registers restored; bit 23 on an Ampere class (`VPRMODE`) untouched |
| 8 | `kf-core Plane` / `kf-trap` token index | the doorbell token's `VECTOR` (the chid) is device-unique (`& 0xFFF`) | `[measured bws4]` UVM_REGISTER_GPU `0x1a` ← *"act birth translated REFUSED: token 0x1: OverDeclaredCap {cap: 0}"* — the PMA scrubber and UVM's first channel were both chid 1 on different runlists. **Blackwell allocates chids per runlist**: `bUsePerRunlistChram` defaults TRUE for every GB1xx/GB20x (`g_kernel_fifo_nvoc.c:226-236`); the token is `RUNLIST_ID 22:16 \| VECTOR 11:0 \| bit 30` (`kernel_fifo_gb202.c:58-78`) | `kf_trap::tokenindex::TokenIndex`: `Vector{mask}` (Turing … Hopper, the old `& mask` byte for byte) or `RunlistVector` (`runlist << 11 \| chid`; 2048 = the per-runlist count we declare); `Family::chids_per_runlist`; births take the runlist the served FIFO table gives the engine; `RC_TRIGGERED` posts the guest chid |
| 9 | `kf-chan translated` (CE offsets) | `OFFSET_{IN,OUT}_UPPER` is 17 bits | `[measured bws4]` **Xid 31 FAULT_PDE** at `0x1fffe_0005_0000` on our ring: the Blackwell host put the identity window at `0x1ff_fffe_0000_0000` (top of a 57-bit VAS) and the rewrite masked it to `NVC7B5`'s `16:0`; `NVC8B5+` is `24:0` (`clc8b5.h:92,96`) | `upper_mask` by the bound CE class |
