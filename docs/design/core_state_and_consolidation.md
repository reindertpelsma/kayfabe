...
| `kayfabe-gsp` | ~3550 | boot FSM, ring + element transport, RPC decode, RAM/replay/fault modules (8 modules) | ★ **BUILT (S0–S5, `f2055bf`)** — this row said "34 / `BootPhase` placeholder enum / **STUB**" until 2026-07-28. The one open *lifecycle* exposure is now narrower: **reboot / suspend-reload only** (S6/S7/S8, hardware-blocked), plus ~~§11-O7a~~ — **RESOLVED 2026-08-06**: CPU-assist events are `emit-never` on BOTH versions (610 did *not* make it local — it re-typed the event; `mode2_gsp_port_plan.md` §11.2). Still un-modeled by choice, and now by a stated contract rather than an open question |
