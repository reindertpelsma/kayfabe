...
| ~~**2**~~ | ★ **ANSWERED 2026-08-06 — NO CHANGE NEEDED, `miss = fault` is already the compliant rule.** Hardware's walk-on-miss *semantics* (an invalid→valid transition needs no invalidate and no notification — `ogkm-580: uvm_va_block.c:6651-6655`, `ogkm-610: uvm_va_block.c:6351`) are reproduced by populating at the guest's **own commit points**, so a dependent access never misses; a genuine miss is what real hardware faults on too, and we deliver it in the driver's format. The C's `runs=0` measurement is the empirical proof that miss-time state really is mid-update. ⚠ **A separate defect WAS found while answering and is fixed** — the walker never consulted `PtPage`/`PdeEdge`'s `aperture`, so a sysmem table page would have its GPA read as a GPGA; now `WalkFault::ForeignAperture`, refused by name. ⊘ And a caveat on my own reasoning: the analysis below was produced against the **two**-transport framing, which the row already corrects — I fed a stale premise into it. The conclusion does not depend on that half. **Original question:** May a miss walk? (`mode2_address_table.md` §6) | a risk posture, not a fact: both designs are implementable and both are safe under some reading. ★ §6's premise is now **false** — a *third* invalidate transport exists (BAR0 PRI `MMU_INVALIDATE`, the Turing+ default, 308 register invalidates in `cap3`), so "the guest commits nothing on that path" is dead. What survives (§6.2 torn multi-level walk) holds for walk-**ahead** and not for walk-**on-fault** | **keep miss = fault for v1.** E5/E8 built the forward-populate path and E8 v2's contested decline turns ambiguity into a loud miss — exactly the diagnosability a walk trades away. Revisit only if the witness requirement is *measured* to be what caps framebuffer performance |
...
| **4** | **The quadratic control plane** — ★ **SHRUNK 2026-08-05, and the owner's first question is why**: *"is this a problem at all, how large is N in normal operation?"* | `[measured]` a real boot to working `nvidia-smi` is **104 allocs and 104 FREES**, live ≤ ~104, 535 request elements — on GA106, GA102 and AD102 identically. The 54.8 s benchmark drives *"alloc+map pairs against a **growing graph**"*, i.e. it never frees: it measures the **hostile** shape. Cost is O(**live**), not O(events), so allocate-and-free never walks the curve. The *"PyTorch reaches it benignly"* line was an **inference** and is struck | **not a redesign.** What remains is a bounded hostile case (a guest that allocates and never frees, to `MAX_LIVE_HANDLES`) — cap it or **partition** the recompute per touched component, which keeps decision #27 intact because each component is still recomputed *wholly from the graph*. ⊘ Still unmeasured: how many objects a CUDA/PyTorch workload holds **live at once** — that, not its total allocations, is the benign ceiling |
| ~~5~~ | ~~**Q12** — the cross-process "verb parked" edge~~ ★ **NO LONGER YOURS — reproduced deterministically 2026-08-05, and it is NOT a protocol change** | `[measured]` shrinking the wait to **0 ms** makes it fail **20/20**, always at the same assertion: `orphans.free` is `[]` where 1 is demanded. The race is that `recv_timeout(TICK).is_err()` proves *"no reply yet"*, while the final assertion needs *"the child already finished `alloc_vaspace`"* — strictly stronger. Abandon lands first ⇒ the chain unwinds with zero intermediates | ⊘ the "protocol change" verdict was reached **without ever reproducing the race**. The park is *induced by the test fixture* (`with_park(ParkVerb::Sysmem)`, a blocking pipe read in the child), so the thing that chooses to park can also **announce** it — a fixture-level progress edge, no isolate-protocol change. Mine to build |
| ~~6~~ | ★ **DONE 2026-08-06 — (a) taken, as you directed** (*"if the citations are wrong just update them or remove, don't leave wrong ones in the doc"*). The gate now covers **every** census row, not just truncated ones, and `if named.is_empty() { continue; }` — the silent skip that let an unattributable citation read as a pass — is a **failure**. 23 citing lines now name their control. ⊘ 14 citations remain out of scope because they name a **body array** or the header banner rather than a registration line; the census cannot resolve those, and that is stated rather than papered over | `#159` found **three miscited oracle rows in shipped source** — every value right, every address wrong, `C:` gate satisfied throughout | ★ measured while doing it: **69** citations, **37** naming no control, and **0** of those 37 in the old scope — so the old gate skipped nothing *it* could see, and every unattributable citation sat outside it |
| 7 | ★ **BOUND FIXED 2026-08-06; POSITION STILL OPEN and now READ rather than summarised.** Your rule applied — *"if it does not break prod apps ever, not even raced, then it's not a thing to assert."* The bound is now `Profile::min_rpc_length()`: **32** at 580, **0** at 610, so the model no longer refuses a zero-length rpc the 610 driver accepts. ⊘ The **position** is unchanged: `ogkm-580: message_queue_cpu.c:760-786` was read and is more specific than "consume and continue" — the driver sets the error, falls through to `exit:`, calls `msgqRxMarkConsumed(nElements)`, bumps `rxSeqNum`, **and then returns the error**. So returning an error is right; returning *without consuming* is the divergence. Not fixed because `nElements` is computed **before** the length test in the driver and **after** it here — and at 610 it derives FROM `rpc_length`, the very field under suspicion. 610's `nElements`-on-failure has **not** been read, and reordering on 580's evidence alone would fix one tag by guessing at the other | one shape change fixes both | ⚠ the new test pins the **value each profile reports**, not that a 610 guest survives a zero-length element end-to-end: the fixture's `send` floors `rpc_length` at 32, so driving that needs raw-element machinery `Guest` does not expose. Stated in the test itself so a green is not read as the larger claim |
