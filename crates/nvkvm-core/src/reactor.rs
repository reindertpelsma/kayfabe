...
//!
//! ## Why this module lives in `nvkvm-core`, not `nvkvm-completion`
//!
...
//! the RM graph); `nvkvm-completion` is the layer *below*, deliberately ignorant of
//! procs — it is pure per-queue policy. Putting the registry there would invert the
//! dependency (`nvkvm-completion` → `nvkvm-core`) and drag the whole graph/projection
//! world under the completion plane. So: the completion **plane** (queues, delivery,
//! fence arms) stays in `nvkvm-completion`; the completion **source routing** is here.
//!
...
///
/// **Scoped to its minting [`SourceRegistry`]** — like [`nvkvm_isolate::HostHandle`]
...
/// use nvkvm_core::reactor::{SourceKind, SourceRegistry};
...
/// becomes guest-driven it needs the [`nvkvm_completion::MAX_OUTSTANDING_COMPLETIONS`]
