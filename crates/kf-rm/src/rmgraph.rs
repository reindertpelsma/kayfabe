...
//! routing — those are pure projections in `kayfabe_core::project` (old tree)).
//!
...
use kf_arch::ids::{ClassId, GpuId, HClient, HObject};
use kf_arch::{ClientKind, ObjectKind, VaSpaceRole};
use kf_chip::Family;
...
/// and `kayfabe_core::project::Boundaries` (old tree) is compared whole across shuffled orders (that IS
...
/// `kayfabe_core::project::ProcBoundary::clients` (old tree) were `HClient`s, so **two generations of one
...
/// value depends on arrival order, and `kayfabe_core::project::Boundaries` (old tree) is compared whole
...
impl ClientKey {
    ...
    /// (`kayfabe_core::gpu::Spine::is_condemned` (old tree)).
    #[must_use]
...
pub struct AllocFacts {
    ...
    /// — it answers [`kf_arch::ids::EngineKind::GrCompute`] for every channel and a CE
    /// channel becomes one later, when its `AMPERE_DMA_COPY_B` object arrives and
    /// `kayfabe_core::project` (old tree)'s refinement pass rewrites it. That is a **derivation standing in
    ...
    pub channel_engine: Option<kf_arch::ids::EngineKind>,
...
///
/// What follows from the reading is that this value and [`kf_arch::PushRange::gpa`]
...
pub struct DeclaredUserd {
    ...
    /// [`kf_arch::UserdMem::Undeclared`], a different fact, and collapsing the two is
    /// the `dlen=0` error.
    pub resolved: Option<kf_arch::UserdMem>,
...
impl DeclaredUserd {
    ...
    pub fn resolved_tag(&self) -> String {
        match self.resolved {
            ...
            Some(kf_arch::UserdMem::Framebuffer { base, size }) => {
                format!("phys=fb:0x{base:x}/0x{size:x}")
            }
            Some(kf_arch::UserdMem::Sysmem { base, size }) => {
                format!("phys=sys:0x{base:x}/0x{size:x}")
            }
            Some(kf_arch::UserdMem::Undeclared { address_space }) => {
...
pub enum RmEvent {
    ...
    Alloc {
        ...
        /// New object's handle.
        handle: HObject,
        /// Class id — classified by the graph's family (`kf_chip::ClassSet::classify`).
        class: ClassId,
    ...
    // ⊘ v3 CUT: `SetPageDir`, `MapMemoryDma`, `Unmap`. They populated the old graph's
    // VA→memory table (`Mapping`/`MapKey`, `backing_of`, `mappings`, `pdb_of`) — an address
    // table, forbidden in v3. The guest's page-directory STATEMENT is now
    // `crate::rmrpc::PageDirStatement`, handed to the memory plane (P4) through
    // `crate::rmrpc::RmObjects::page_dir`; mapping RPCs are the RPC-map synchronization point,
    // served by that plane, not recorded here.
...
pub struct RmNode {
    ...
    pub class: ClassId,
    /// Family-classified kind (recorded at apply; classification is stable per family).
    pub kind: ObjectKind,
...
struct Resource {
    ...
    /// device's) free. Without this, a UVM dup that
    ...
    /// only available answer was an absence, and `kayfabe_core::project` (old tree)'s `anchor_of` read
    ...
    /// order-independent *label* `kayfabe_core::project` (old tree) may report, where [`Self::owner`] is
...
pub enum RmGraphError {
    ...
    /// client of **unknown [`kf_arch::ClientKind`]**, which is precisely the guess
    ...
    FreeUnknown(NodeKey),
    /// ★ v3 — an ENGINE class this graph's family does not list (see
    /// [`RmGraph::engine_class_refusal`]).
    EngineClassNotInFamily {
        /// The class the guest asked for.
        class: ClassId,
        /// This graph's family.
        family: Family,
    },
...
#[derive(Debug, Clone)]
pub struct RmGraph {
    /// ★ v3 — the family whose class set classifies every alloc ([`kf_chip::ClassSet::classify`])
    /// and admits engine classes ([`RmGraph::engine_class_refusal`]). Replaces the old
    /// `&dyn Arch` parameter to [`RmGraph::apply`], which was implemented per die group
    /// (`Ga10xArch` classified Ampere literals only).
    family: Family,
    ...
    /// is never gated: last declaration wins is
...
impl RmGraph {
    ...
    ///
    /// ⊘ No `Default`: a graph with no family would classify every engine class as unknown.
    #[must_use]
    ...
    /// The family this graph classifies for.
    #[must_use]
    pub fn family(&self) -> Family {
        self.family
    }

    /// ★ Classify `class` on this graph's family — every family first-class, the class sets
    /// generated from ogkm ([`kf_chip::classes`]).
    #[must_use]
    pub fn classify(&self, class: ClassId) -> ObjectKind {
        self.family.classes().classify(class)
    }

    /// ★★★ **The per-family engine-class gate** — merged from the old doorbell graph's
    /// `class_policy_on` (`V3_P2_PORT_MAP.md` §1, doorbell rmgraph.rs: "merge its class
    /// allowlist").
    ///
    /// An ENGINE class (channel, compute, copy, usermode, 3D) that some family lists but
    /// **this** family's chips do not is refused here, by name, rather than bounced off the
    /// host RM later: a Turing guest asking for `HOPPER_COMPUTE_A` never reaches a host call.
    /// Non-engine classes are not this gate's business — the driver-version capability table
    /// (`kf_abi::capability`, checked in [`crate::rmrpc::translate`]) is the default-deny for
    /// those, and already refuses `NV01_MEMORY_LOCAL_PRIVILEGED`, `NV01_MEMORY_SYSTEM_OS_DESCRIPTOR`
    /// and `NV40_I2C` by name.
    ///
    /// ⊘ Fixes the old doorbell graph's defect 2 (`V3_P2_PORT_MAP.md` §4.4): its `alloc` called
    ...
    /// the handle-shaped public API (`pdb_of`-family lookups (old tree; cut in v3), [`Self::references`], …), whose
    ...
    /// declaration's *death* — `kayfabe_core::gpu::Spine::condemned` (old tree) — keys on [`ClientId`],
    ...
    fn clients_named(ev: RmEvent) -> [Option<HClient>; 2] {
        match ev {
            RmEvent::Alloc { client, .. } | RmEvent::Free { client, .. } => [Some(client), None],
    ...
    /// - **`Free` — the TEARDOWN verb** (the old `Unmap` is cut with the mapping table). A namespace with no root is
    ...
    fn undeclared_namespace(&self, ev: RmEvent) -> Option<HClient> {
        ...
        match ev {
            ...
            RmEvent::Alloc { class, .. } if matches!(self.classify(class), ObjectKind::Client) => {
                None
            }
            RmEvent::Alloc { client, .. } => missing(client),
            ...
            // Teardown verb — see the doc above.
            RmEvent::Free { .. } => None,
        }
    }
    ...
    fn walk_gpu(&self, key: NodeKey) -> Option<GpuId> {
        ...
        for _ in 0..64 {
            if matches!(node.kind, kf_arch::ObjectKind::Device) {
    ...
    pub fn apply(&mut self, ev: RmEvent) -> Result<(), RmGraphError> {
        ...
        if let Some(c) = self.undeclared_namespace(ev) {
        ...
        match ev {
            RmEvent::Alloc {
                ...
                if let Some(refusal) = self.engine_class_refusal(class) {
                    return Err(refusal);
                }
                ...
                match self.handles.get(&key) {
                    ...
                    None => {
                        ...
                        if matches!(self.classify(class), kf_arch::ObjectKind::Device) {
                            self.cache_targets();
                        }
                        Ok(())
                    }
    ...
    fn free_subtree(&mut self, key: NodeKey) {
        ...
        //
        // The retain above prunes by membership in `doomed`, which is the set of
    ...
    /// finding 4 — shared by `pdb_of`-family lookups (old tree; cut in v3), `pdb_of`-family lookups (old tree; cut in v3), `map_ref_count` (old tree; cut in v3)
    ...
    /// silently disarmed the F1 `kayfabe_core::project::ProjectionError::PdbCollision` (old tree) guard
    /// for that value).
    ///
    ...
    ///
    /// Exactly `pdb_of`-family lookups (old tree; cut in v3)'s argument: a handle-shaped lookup answers about
    ...
    ///
    /// **MISS ⇒ DEFER**, exactly as `pdb_of`-family lookups (old tree; cut in v3): a Device legitimately exists before
...
impl crate::rmrpc::Faulted for RmGraphError {
    fn fault_tag(&self) -> kf_trace::FaultTag {
