...
//!
//! [`POLICY_DISPOSITIONS`] is one row per [`kf_gsp::CommandPolicy`] implementation in
...
pub fn reply_would_be_cached(cmd: u32, flags: u32, access_right: u32) -> bool {
    kf_abi::capability::RM_GSS_LEGACY_MASK & cmd != 0
...
pub const POLICY_DISPOSITIONS: [PolicyDisposition; 16] = [
    ...
    // `NeverAnswers` row until §14.41, when it became a `kf_gsp::CommandObserver` — a
    ...
    // ★★★ §14.23 — the observer ADAPTER (`kf_gsp::Observing`), and its row is the
...
kf_util::assert_send!(StickyAnswerGuard);
kf_util::assert_send_sync!(PolicyDisposition, StickyDisposition);
