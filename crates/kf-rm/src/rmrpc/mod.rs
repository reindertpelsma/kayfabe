//! # `kf_rm::rmrpc` — the GSP → object-model bridge (from `kayfabe-rmrpc`)
//!
//! `docs/design/gsp_core_bridge.md`. One decoded GSP RPC ([`kf_gsp::RpcCommand`])
//! becomes one declared protocol fact ([`crate::rmgraph::RmEvent`]), a guest page-directory
//! statement for the memory plane ([`PageDirStatement`]), a known-and-inert nothing, or a
//! **named refusal**. Nothing else.
//!
//! ## ⊘ v3 port notes (`V3_P2_PORT_MAP.md` §1, kayfabe-rmrpc rows)
//!
//! - **Kept:** [`translate`] and its arms (`translate_alloc`, `_control`, `_dup`, `_free`,
//!   `translate_published_pdes`), [`Reassembler`] (copied), the [`GraphPolicy`] dispatch and
//!   the refusal wire format ([`BridgeRefusal::rpc_result`], `refusal_reply`).
//! - **Cut:** `Translation::CtxPromotion` + `translate_promote_ctx` — the promote JOIN (VA →
//!   physical bindings into an address table). `GPU_PROMOTE_CTX` is now refused by name
//!   ([`BridgeRefusal::PromoteCtxNotModelled`]) until the channel plane serves it.
//!   `ObjectModel` (built on the old `Gpu`/`Proc` spine and isolates) is replaced by the narrow
//!   [`RmObjects`] trait a later crate implements; `PublicationObserver`, the ring and promote
//!   censuses, the `KAYFABE_SUBDEV_FWD` env gate, and `fault.rs` (RC emission, P5.5) are not
//!   carried. The channel-plane controls (`OBJECT_CONTROLS`: schedule, bind, preempt,
//!   ctxsw-mode, MC service, relays) are P5.
//! - **Changed:** the old `RmEvent::SetPageDir` is now [`Translation::PageDir`] carrying a
//!   [`PageDirStatement`], handed to [`RmObjects::page_dir`] — the graph holds no PDB.
//!
...
//! - [`GraphPolicy`] — B2 ([`policy`]). The `kf_gsp::CommandPolicy` the boot FSM
...
//! `Disposition` is computed in `kf_gsp::GspFsm::answer` from the arriving function
...
//! absorbed.**
//!
//! ★ **B3 is the class table** ([`kf_abi::versions::DriverAbiTable::alloc_params`]):
...
pub use policy::{
    GraphObjects, GraphPolicy, OBJECT_VERBS, ObjectPolicy, ObjectsRefusal, RefusalCensus,
    RmObjects, SharedRefusalCensus,
};
...
/// Names the exact refusal a fault type represents — the old `kayfabe_trace::Faulted`, which
/// `kf-trace` does not carry (it has [`FaultTag`] only). ⊘ Declared here rather than added to
/// kf-trace so this port touches no crate outside kf-rm; it can move down when a second crate
/// needs it. Every impl must be an exhaustive `match`.
pub trait Faulted {
...
///
/// ⊘ v3: `Copy` again — the old `CtxPromotion` variant (a `Vec` of declared ranges, the
/// promote join) is cut.
...
pub enum Translation {
    ...
    Held,
    /// ★★ **The guest's page-directory STATEMENT** — `SET_PAGE_DIRECTORY` (`0x00801813`) or
    /// a reserved-PDE publication (`0x90f10106` / `0x20800a9f`).
    ///
    /// ⊘ v3: this was `RmEvent::SetPageDir`, applied to the graph, which stored the PDB on the
    /// VA-space resource and served `pdb_of` lookups — half of the old VA→memory address
    /// table (`V3_P2_PORT_MAP.md` §1 cuts `pdb_of`). The statement is now a fact for the
    /// memory plane (P4), handed over by [`RmObjects::page_dir`]; the object graph holds no
    /// PDB at all.
...
///
/// Recorded as an attribute of the VA-space object by the memory plane (P4). It is a
/// STATEMENT: nothing here reads the tables it points at (v3: no CPU reads of guest page
/// tables).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct PageDirStatement {
    ...
    pub pdb: Pdb,
    /// ★★★★★ **WHICH ADDRESS SPACE `pdb` IS IN.** `None` means the guest published a value
    /// we do not understand (`PdbAperture::Undefined(n)`); it must NOT collapse to vidmem —
...
pub enum BridgeRefusal {
    ...
    /// ★★★ **The boundary refuses this class outright** — [`kf_abi::capability`],
    ...
    ///
    /// The configured [`GuestOs`] profile has no [`kf_abi::ClientKindRule`], so
    /// `NV0000_ALLOC_PARAMETERS.processID` cannot be turned into a
    /// [`kf_arch::ClientKind`] — and this refuses instead of picking one.
    ///
    ...
    /// - **this** — [`kf_abi::capability::PassthroughRule`] put it in scope, and the
    ...
    ///   [`kf_abi`] table row, not a logic-crate edit, so it costs no change here) —
    ...
    /// |---|---|---|
    /// | the RPC **envelope**'s `rpc_result` | [`kf_gsp::Reply::rpc_result`] | `_issueRpcAndWait` returns non-`NV_OK` (`ogkm-580: rpc.c:1994`, `ogkm-610: :2012`), so the entire `if (status == NV_OK) { … }` block is skipped |
    ...
    /// [`kf_abi::NV_ERR_NOT_SUPPORTED`] for every refusal, so the guarantee is
    ...
    /// [`kf_abi::versions::ControlParams::PageDirNotModelled`]. The headline is
    ...
    ///
    /// [`kf_abi::versions::ControlParams::VaspacePublishedPdes`]'s whole point is
    ...
    /// see [`kf_abi::gvaspacepdes::ServerReservedPdesError`], every variant of which
    ...
    PublishedPdesMalformed {
        ...
        err: kf_abi::gvaspacepdes::ServerReservedPdesError,
    },
    ...
    /// [`kf_arch::ids::Pdb`] is documented as a per-GPU **FB** address, so a
    ...
    /// guest-RAM store. So this carries the **decoded** [`kf_abi::view::PdbAperture`]
    ...
    SetPageDirRootAperture {
        ...
        aperture: kf_abi::view::PdbAperture,
    },
    ...
    ///
    /// [`ObjectsRefusal`] is carried whole, and [`Faulted`] **delegates** to it, so the
    /// census records *which protocol rule* was broken (`RmGraphError::FreeUnknown` vs
    /// `::ConflictingAlloc` are different findings) rather than one flat "the graph said no".
    ///
    /// ⊘ v3: was `Graph(GpuError)`, the old `Gpu`/`Proc` spine's error.
    Objects(ObjectsRefusal),
    /// ⊘ v3 — `GPU_PROMOTE_CTX` (`0x2080012b`), refused by name. The old arm translated it
    /// into `Translation::CtxPromotion` and joined its VA → physical bindings into an address
...
impl BridgeRefusal {
    ...
    /// [`kf_abi::NV_ERR_NOT_SUPPORTED`] and
    /// [`kf_abi::NV_VGPU_MSG_RESULT_VMIOP_BASE`]; the three facts that decided it:
    ///
...
impl Faulted for BridgeRefusal {
    ...
    fn fault_tag(&self) -> FaultTag {
        match self {
            ...
            // ★ Delegated, not flattened, so a graph refusal is countable by the rule it
            // broke. The refusal VALUE still says it came through the bridge; the tag says
            // what actually went wrong, which is what a census is for.
            BridgeRefusal::Objects(e) => e.fault_tag(),
            BridgeRefusal::PromoteCtxNotModelled { .. } => {
...
pub fn translate(
    ...
    match cmd.function {
        ...
        // (`kf_gsp::RpcFunction::disposition`).
...
/// ## The class table
///
/// Which params shape a class carries is [`kf_abi::versions::DriverAbiTable::alloc_params`]'s
...
fn translate_alloc(
    ...
    // silently widens the boundary. Default-deny (`kf_abi::capability`).
    ...
    let facts = match shape {
        ...
        AllocParams::VaSpace => AllocFacts {
            vaspace_role: abi.decode_vaspace_index(params).map(|index| {
                if index == kf_abi::bringup::NV_VASPACE_ALLOCATION_INDEX_GPU_DEVICE {
                    kf_arch::VaSpaceRole::DeviceDefault
                } else {
                    kf_arch::VaSpaceRole::Own
                }
            }),
            ..Default::default()
        },
        AllocParams::Channel => {
            ...
            AllocFacts {
                ...
                // (`kf_abi::notifier::ChannelNotifierWire`). `Ok(None)` at an
                ...
                // (`kf_abi::notifier::ChannelUserdWire`). ⊘ Read by no decision; it
                ...
                gp_fifo_ring: Some(crate::rmgraph::GpFifoRing {
                    va: c.gp_fifo_offset,
                    entries: c.gp_fifo_entries,
                }),
                // ★★★★★ THE ENGINE THE GUEST DECLARED, decoded past the prefix for the
                // notifier's reason and by the same mechanism
                // (`kf_abi::notifier::ChannelEngineWire`). Until this line the engine
...
/// `GSP_RM_CONTROL` (fn 76) → [`Translation::PageDir`], for exactly one `cmd`.
///
...
///
/// [`kf_abi::versions::DriverAbiTable::control_params`] is the table; this function
...
/// [`Translation::PageDir`] has nowhere to put it, and the VASpace is named by a params
/// field, not by it.
///
...
/// address spaces — and `kf_arch::ids::Pdb`'s own doc currently assumes the first
...
fn translate_control(abi: &DriverAbiTable, payload: &[u8]) -> Result<Translation, BridgeRefusal> {
    ...
    if shape == ControlParams::PromoteCtx {
        return Err(BridgeRefusal::PromoteCtxNotModelled { cmd: h.cmd });
    }
    ...
    // `kf_abi::view::PdbAperture::from_flags`'s, so the two encodings never meet.
    if p.aperture != kf_abi::view::PdbAperture::Vidmem {
        return Err(BridgeRefusal::SetPageDirRootAperture {
            aperture: p.aperture,
        });
    }
    Ok(Translation::PageDir(PageDirStatement {
        client,
...
/// `ROUTE_TO_PHYSICAL` twin `0x20800a9f` → [`Translation::PageDir`].
///
...
/// | the PDB | [`ServerReservedPdes::root()`](kf_abi::gvaspacepdes::ServerReservedPdes::root)`.phys_address` | index 0 is the root; the derivation and its corroborating half are on that method |
///
...
fn translate_published_pdes(
    ...
    // ⊘ The decoder is `kf_abi`'s and it VALIDATES: every refusal it can return is a
    ...
    if root.aperture != kf_abi::gvaspacepdes::GMMU_APERTURE_VIDEO {
...
kf_util::assert_send_sync!(Translation, BridgeRefusal);
