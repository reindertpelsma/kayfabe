...
//! ([`crate::hostfacts::HostFacts::engines`], a fact about silicon) and the *layout*
//! (`kf_abi::inittables`, the Axis-A quarantine). This crate is the adapter where a
...
use kf_abi::pcibars::{self, NV2080_CTRL_CMD_BUS_GET_PCI_BAR_INFO, PCI_BAR_INFO_PARAMS_SIZE};
use kf_abi::regaccessmap::{
...
pub struct InitTablePolicy {
    /// The board we present (PCI identity, BAR table).
    board: Arc<BoardFacts>,
    /// ★ The host die's facts — every value that used to be a `ChipProfile` row.
    host: Arc<HostFacts>,
    ...
    /// arm although [`kf_abi::eventnotify::SILENT_NOTIFIERS`] does not admit them.
    ...
    /// [`kf_abi::gspfeatures::FirmwareVersion`] at the latch and dropped if they fail.
    ...
    guest_firmware: Option<kf_abi::gspfeatures::FirmwareVersion>,
}
...
pub enum WantedTable {
    ...
    /// `kf_abi::pcibars`' module docs carry the dmesg, and
    ...
    /// [`kf_abi::regaccessmap`], and [`crate::ga10x::GA106_USER_REGISTER_ACCESS_MAP`]
    ...
    /// [`Self::UserRegisterAccessMap`]'s. See [`kf_abi::falconinfo`], and
    ...
    /// [`kf_abi::memsysconfig`], which makes both unencodable.
    ...
    /// to tell the two apart. See [`kf_abi::confcompute`], which makes the widening
    ...
    /// framebuffer and a PCI function 1 — and [`kf_abi::bifstatic`] makes both
    ...
    /// first time — see [`kf_abi::gmmustatic`], which states that boundary rather than
    /// eliding it.
    GmmuStaticInfo,
    ...
    /// [`kf_abi::oracle`] — and it needs none, because there is no `[OUT]` value to be
    ...
    ///
    /// Decided on evidence in [`kf_abi::faultbuffer`]'s module docs — the three-line
    ...
    /// deliberate choice, and built), is in [`kf_abi::faultbuffer`], and
    /// [`kf_abi::faultbuffer::SHADOW_DELIVERY_UNBUILT`] is what the boot report prints.
    ...
    /// exactly. ⊘ The unbuilt half is [`kf_abi::faultbuffer::ACCESS_COUNTER_DELIVERY_UNBUILT`]
    ...
    /// [`HostFacts`] row. This one answers a **verb on the
    ...
    /// [`kf_abi::eventnotify`], which carries the full reading and the
    /// [`SILENT_NOTIFIERS`](kf_abi::eventnotify::SILENT_NOTIFIERS) scope that decides
    ...
    /// [`kf_abi::l2evict`], which carries the full argument, the falsifiers, and the
    ...
    ///
    /// [`crate::hostfacts::HostFacts::ce_fault_method_buffer_size`]. `[measured]` 20480 on a real
    /// RTX 3060; the argument for measuring rather than choosing, and for a chip row rather
    /// than a constant, is in [`kf_abi::fmbsize`].
    ///
    ...
    /// about the shader core. See [`kf_abi::grstatic`] for why these five and not the
    ...
    /// (`kf_abi::versions`), and `encode_message`'s `max_elements` guard is what says
    ...
    /// for the split VA space. See [`kf_abi::gvaspacepdes`].
    ///
    ...
    /// scrubber to this one — see [`kf_abi::gvaspacepdes`] for the ten-line cascade
    ...
    /// identity indices this port refuses by name are in [`kf_abi::gpuinfo`].
    ///
    ...
    /// # The value
    ///
    /// [`crate::hostfacts::HostFacts::smc_mode`], `[measured]` `Unsupported` on two physical GA106
    /// parts by two different instruments. ⊘ **Not** from the C oracle, whose row for this id
    /// is one of the eleven `dlen = 0` rows; the argument is in [`kf_abi::smcmode`].
    ...
    /// one enum — [`crate::hostfacts::HostFacts::pcie_max_gen`] — and the word is DERIVED from it by
    /// [`kf_abi::businfo::PcieGenInfo::fully_trained`]. The whole measurement, and the
    ...
    /// this zero is not the `0x20802a08` zero are in [`kf_abi::gpuatomics`].
    ///
    ...
    /// [`kf_abi::fbinfo`] for the whole refutation.
    ///
    ...
    /// **projections of [`crate::hostfacts::HostFacts::memory_system`]**, the row already served to
    ...
    /// Every byte is a projection of [`crate::hostfacts::HostFacts::engines`], the same slice
    ...
    /// `NV_CE_GRCE_ALLOWED_LCE_MASK` intersected with them. See [`kf_abi::cecaps`] for
    ...
    /// the identical [`kf_abi::cecaps::CeGeometry`] row [`Self::CeGetAllPhysicalCaps`]
    ...
    /// drift [`kf_abi::deviceinfo`] exists to forbid.
    ///
    ...
    /// [`kf_abi::cecaps`] measured through two other callers, corroborated here by a
    /// third control that shares no code with them.
    ///
    /// ⊘ The value is the chip row's ([`crate::hostfacts::HostFacts::lce_pce_masks`]), not a constant
    ...
    /// [`kf_abi::grfsinfo`] refuses per-query only where RM itself does, and takes the
    ...
    /// here can only ever have come off the RPC. See [`kf_abi::gspfeatures`] for why
    ...
    /// out in [`kf_abi::gspfeatures`]'s module docs.
    ///
    ...
    /// [`kf_abi::gspfeatures`]'s docs: the answer is constant for the life of a
    ...
    /// `SKIP_COPYOUT` flag exists on it. [`kf_abi::gsslegacy`] carries the argument, and
    ...
    /// [`crate::sticky::StickyAnswerGuard`]'s — see [`kf_abi::gsslegacy`].
    GssLegacy8162,
    ...
    /// argument survives the capture being deleted. See [`kf_abi::c2cinfo`].
    C2cInfo,
    ...
    /// accepted. See [`kf_abi::fmbpromote`] for the honesty question re-asked per id, for
    ...
    /// guest, no emulator and no QEMU. See [`kf_abi::cudartinit`] for the trace
    ...
    /// where our guest is. See [`kf_abi::cudartinit::CUDART_INIT_0XA001`].
    CudartInit9A001,
...
impl WantedTable {
    ...
    pub fn cmd_id(self) -> u32 {
        match self {
            ...
            Self::RegisterFaultBuffer => {
                kf_abi::faultbuffer::NV2080_CTRL_CMD_INTERNAL_GMMU_REGISTER_FAULT_BUFFER
            }
            Self::RegisterClientShadowFaultBuffer => {
                kf_abi::faultbuffer::NV2080_CTRL_CMD_INTERNAL_GMMU_REGISTER_CLIENT_SHADOW_FAULT_BUFFER
            }
            Self::RegisterAccessCntrBuffer => {
                kf_abi::faultbuffer::NV2080_CTRL_CMD_INTERNAL_UVM_REGISTER_ACCESS_CNTR_BUFFER
            }
            Self::EventSetNotification => {
            ...
            Self::GrInfo => kf_abi::grinfo::NV2080_CTRL_CMD_INTERNAL_STATIC_KGR_GET_INFO,
            Self::GrFloorsweepingMasks => {
            ...
            Self::InternalGpuGetSmcMode => {
                kf_abi::smcmode::NV2080_CTRL_CMD_INTERNAL_GPU_GET_SMC_MODE
            }
            Self::BusGetInfoV2 => kf_abi::businfo::NV2080_CTRL_CMD_BUS_GET_INFO_V2,
            Self::BusGetPcieSupportedGpuAtomics => {
                kf_abi::gpuatomics::NV2080_CTRL_CMD_BUS_GET_PCIE_SUPPORTED_GPU_ATOMICS
            }
            Self::FbGetInfoV2 => kf_abi::fbinfo::NV2080_CTRL_CMD_FB_GET_INFO_V2,
            Self::CeGetAllPhysicalCaps => {
                kf_abi::cecaps::NV2080_CTRL_CMD_CE_GET_ALL_PHYSICAL_CAPS
            }
            Self::CeGetPhysicalCaps => kf_abi::cecaps::NV2080_CTRL_CMD_CE_GET_PHYSICAL_CAPS,
            Self::CeGetCePceMask => kf_abi::cepce::NV2080_CTRL_CMD_CE_GET_CE_PCE_MASK,
            Self::GrmgrGetGrFsInfo => kf_abi::grfsinfo::NV2080_CTRL_CMD_GRMGR_GET_GR_FS_INFO,
            Self::GspGetFeatures => kf_abi::gspfeatures::NV2080_CTRL_CMD_GSP_GET_FEATURES,
            Self::GssLegacy8159 => kf_abi::gsslegacy::GSS_LEGACY_0X8159,
            Self::GssLegacy8162 => kf_abi::gsslegacy::GSS_LEGACY_0X8162,
            Self::CudartWatchdogInfo => kf_abi::cudartinit::RC_GET_WATCHDOG_INFO,
            Self::CudartInit9009 => kf_abi::cudartinit::CUDART_INIT_0X9009,
            Self::CudartInit9001 => kf_abi::cudartinit::CUDART_INIT_0X9001,
    ...
    pub fn params_size(self) -> usize {
        match self {
            ...
            Self::GrInfo => kf_abi::grinfo::KGR_GET_INFO_PARAMS_SIZE,
            ...
            Self::FbGetInfoV2 => kf_abi::fbinfo::FB_GET_INFO_V2_PARAMS_SIZE,
            Self::CeGetAllPhysicalCaps => kf_abi::cecaps::CE_GET_ALL_CAPS_PARAMS_SIZE,
            Self::CeGetPhysicalCaps => kf_abi::cecaps::CE_GET_CAPS_V2_PARAMS_SIZE,
            Self::CeGetCePceMask => kf_abi::cepce::CE_GET_CE_PCE_MASK_PARAMS_SIZE,
            Self::GrmgrGetGrFsInfo => kf_abi::grfsinfo::GR_FS_INFO_PARAMS_SIZE,
            Self::GspGetFeatures => kf_abi::gspfeatures::GSP_GET_FEATURES_PARAMS_SIZE,
            Self::GssLegacy8159 => kf_abi::gsslegacy::GSS_LEGACY_0X8159_PARAMS_SIZE,
            Self::GssLegacy8162 => kf_abi::gsslegacy::GSS_LEGACY_0X8162_PARAMS_SIZE,
...
impl InitTablePolicy {
    ...
    #[must_use]
    pub fn guest_firmware(&self) -> Option<kf_abi::gspfeatures::FirmwareVersion> {
        self.guest_firmware
    }
...
impl CommandPolicy for InitTablePolicy {
    fn respond(&mut self, cmd: &RpcCommand) -> Option<Reply> {
        ...
        //
        // ⊘⊘ **This port refuses them today, and `kf_abi::gsslegacy`'s module doc cites
        ...
        // measured — precisely the truncated-row hazard `kf_abi::oracle` was written for.
        ...
        let params = match want {
            ...
            WantedTable::ChipInfo => {
                // ⊘ v3: the identity is the BOARD's (the host's own PCI identity, so the guest
                // driver binds), not `identity_for(chip)` — which cross-checked a chip row
                // against a VBIOS row. One source now, so nothing to cross-check.
                let b = &self.board;
                let id = ChipIdentity {
                    pci_vendor_id: b.pci_vendor_id,
                    pci_device_id: b.pci_device_id,
                    pci_subsystem_vendor_id: b.pci_subsystem_vendor_id,
                    pci_subsystem_id: b.pci_subsystem_id,
                    pci_revision: b.pci_revision,
                };
            ...
            // userspace" unencodable (`kf_abi::regaccessmap`), and answering anyway
            ...
            // (`kf_abi::falconinfo::FalconInfoError::TooManyFalcons`) — unencodable.
            ...
            // (`ogkm-580: gpu.c:2170-2214`, and `kf_abi::memsysconfig`'s docs for the Oops
            ...
            // silicon is the drift `kf_abi::deviceinfo` exists to forbid.
            //
            ...
            // device AND what a refusal leaves behind — see `kf_abi::confcompute` —
            ...
            // instead and reported; see `kf_abi::faultbuffer`.
            ...
            //    arming for one that cannot occur. See `kf_abi::eventnotify`.
            ...
            // an echo — is in `kf_abi::l2evict`; it is deliberately not restated,
            ...
            WantedTable::CeFaultMethodBufferSize => {
                match kf_abi::fmbsize::encode_fault_method_buffer_size(
            ...
            WantedTable::GrInfo => {
                if self
                    .host
                    .gr_info
                    .validate_against(&self.host.gr_static)
                    .is_err()
                {
                    return refuse();
                }
            ...
            // `kf_abi::grstatic`'s deferred-half section for why pretending they could
            ...
            // `kf_abi::smcmode` for the two-part provenance.
            WantedTable::InternalGpuGetSmcMode => {
                kf_abi::smcmode::encode_smc_mode(self.host.smc_mode)
            }
            ...
            WantedTable::BusGetPcieSupportedGpuAtomics => {
                let at = req.params_at;
                match kf_abi::gpuatomics::answer_bus_get_pcie_supported_gpu_atomics(
                    &cmd.payload
                        [at..at + kf_abi::gpuatomics::PCIE_SUPPORTED_GPU_ATOMICS_PARAMS_SIZE],
            ...
            // NUMBER: all four values are projections of `self.host.memory_system`, the row
            ...
            // relations named with their architecture in `kf_abi::fbinfo`. A second
            ...
            WantedTable::CeGetPhysicalCaps => {
                ...
                let at = req.params_at;
                match kf_abi::cecaps::answer_ce_get_physical_caps(
                    &cmd.payload[at..at + kf_abi::cecaps::CE_GET_CAPS_V2_PARAMS_SIZE],
                    &geometry,
                ) {
                    Ok(p) => p,
                    Err(e) => return refuse_named(req.cmd, &e),
            ...
            WantedTable::CeGetCePceMask => {
                ...
                let at = req.params_at;
                match kf_abi::cepce::answer_ce_get_ce_pce_mask(
                    &cmd.payload[at..at + kf_abi::cepce::CE_GET_CE_PCE_MASK_PARAMS_SIZE],
                    geometry.present,
            ...
            // neither ledger would carry a word about it. `kf_abi::grfsinfo` therefore
            ...
            // `self.driver.version()` = 580.65.06) are laid out in `kf_abi::gspfeatures`.
            //
            ...
            WantedTable::GspGetFeatures => {
                ...
                kf_abi::gspfeatures::encode_gsp_get_features(
                    ...
                    // part — the same fact `kf_abi::smcmode` reports as `Unsupported`.
                    self.host.gsp_features,
            ...
            // is the request VERBATIM. See `kf_abi::gsslegacy` for why that is a
            ...
            // conditions in `kf_abi::cudartinit`.
            ...
            WantedTable::GssLegacy8159 | WantedTable::GssLegacy8162 => {
                let at = req.params_at;
                match kf_abi::gsslegacy::answer_gss_legacy(
                    req.cmd,
                    &cmd.payload[at..at + want.params_size()],
                ) {
                    Ok(p) => p,
            ...
            //
            // ⊘⊘ **This line shipped INVERTED (`!self.host.has_c2c`) and a boot caught it, not
            // a test.** `kf_abi::c2cinfo`'s own unit tests were green — both arms of
