...
fn a_test_gid() -> kf_abi::gspstaticinfo::GpuGid {
    kf_abi::gspstaticinfo::GpuGid::derive(b"gsp_static_info layout test")
}
...
fn chip() -> kf_rm::BoardFacts {
    ga106::board_at(ga106::FB_SIZE_MB)
}

fn policy() -> StaticInfoPolicy {
    StaticInfoPolicy::new(ga106::board(), *table_for(BENCH_DRIVER).expect("bench ABI"))
}

/// ⊘ v3 has no `CHIPS` table: the old gates quantified over the chip rows. Here they quantify
/// over the boards the fixture can present — GA106 at its compiled size and at a smaller
/// operator-chosen size (constraint 15: the size is the store's) — named for the messages.
...
///
/// ⚠ Quantified over the fixture's boards (`gates_quantified_over_a_list`); in v3 the name
/// arrives only as `HostFacts::gpu_name` through the served chain, never through a board row.
#[test]
...
fn the_three_statements_of_the_framebuffer_size_agree() {
    ...
    // ⊘ The third statement — the `NV_USABLE_FB_SIZE_IN_MB` boot register the old chip row
...
fn the_two_installed_policies_do_not_both_claim_a_function() {
    ...
    let mut tables = kf_rm::inittables::InitTablePolicy::new(ga106::board(), ga106::host(), driver);
    let mut static_info = StaticInfoPolicy::new(ga106::board(), driver);
    assert!(
        tables
            .respond(&command(RpcFunction::GetGspStaticInfo, 65, 1792))
            .is_none(),
...
fn a_chip_whose_table_cannot_be_encoded_is_refused_in_the_envelope() {
    ...
    let bad = kf_rm::BoardFacts {
        fb_regions: BAD_REGIONS.to_vec(),
        fb_length: 0x3_0000_0000,
        bar1_pde_base: ga106::bar1_pde_base_for(ga106::FB_SIZE_MB),
        ..chip()
    };
    let driver = *table_for(BENCH_DRIVER).expect("bench ABI");
    let p = StaticInfoPolicy::new(std::sync::Arc::new(bad), driver);
    assert_eq!(
