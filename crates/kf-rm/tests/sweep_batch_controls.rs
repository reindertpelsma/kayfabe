...
fn params_of(reply: &kf_gsp::Reply, len: usize) -> Vec<u8> {
...
fn a_trust_claim_this_port_cannot_back_is_unencodable() {
    ...
    for (bar1, pcie) in [(true, false), (false, true), (true, true)] {
        let row = kf_abi::confcompute::ConfComputeRow {
            bar1_trusted: bar1,
            pcie_trusted: pcie,
        };
        assert_eq!(
...
fn the_bif_reply_is_the_oracles_four_bytes() {
    let ours = bifstatic::encode_bif_static_info(&ga106::host_facts().bif_static).expect("encodes");
...
fn the_two_bif_bits_that_point_rm_at_absent_hardware_are_unencodable() {
    ...
    let base = ga106::host_facts().bif_static;
    assert_eq!(
        bifstatic::encode_bif_static_info(&kf_abi::bifstatic::BifStaticRow {
            c2c_link_up: true,
            ..base
        }),
    ...
    // reason.
    let gen4 = bifstatic::encode_bif_static_info(&kf_abi::bifstatic::BifStaticRow {
...
fn a_zero_channel_count_is_unencodable() {
    ...
    assert_eq!(
        fifochannels::encode_fifo_num_channels(
            &kf_abi::fifochannels::FifoChannelsRow {
    ...
    assert!(fifochannels::encode_fifo_num_channels(&ga106::host_facts().fifo_channels, 0).is_ok());
}
...
fn a_fault_buffer_geometry_rm_would_reject_is_unencodable() {
    ...
    let base = ga106::host_facts().gmmu_static;
    assert_eq!(
        gmmustatic::encode_gmmu_static_info(&kf_abi::gmmustatic::GmmuStaticRow {
