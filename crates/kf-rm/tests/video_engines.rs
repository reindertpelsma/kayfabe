...
/// ★ v3-int: the GRCE mask the families side made an explicit input (host-derived CE caps);
/// GA10x's, as the captured GA106 rows these tests pin were taken with.
...
fn the_video_rows_carry_the_real_ga106s_ogkm_constants() {
    let rows = authored::engine_table(Family::Ampere, &ga106_kinds(), GRCE).expect("Ampere states every video constant");
...
fn the_video_fault_ids_follow_each_familys_dev_fault_h() {
    ...
    let t = authored::engine_table(Family::Turing, &kinds(1, 2), GRCE).expect("Turing");
    ...
    let a = authored::engine_table(Family::Ada, &kinds(3, 4), GRCE).expect("Ada");
    assert_eq!((fault(&a, "NVENC2"), fault(&a, "NVDEC3")), (Some(13), Some(28)));
    // Blackwell: NVENC 44.., NVDEC 28.. (blackwell/gb202/dev_fault.h).
    let b = authored::engine_table(Family::Blackwell, &kinds(4, 4), GRCE).expect("Blackwell");
    ...
    let e = authored::engine_table(Family::Ampere, &kinds(4, 0), GRCE).expect_err("Ampere names no NVENC3");
...
fn engine_caps_sets_the_nv2080_bits_of_exactly_the_served_engines() {
    ...
    let old = authored::engine_table(Family::Ampere, &ga106_kinds()[..5], GRCE).expect("Ampere");
