...
/// reading well: it is checked against `kf_abi::oracle`'s censused `kept`, and against
...
fn every_oracle_byte_this_file_reads_is_inside_what_the_recorder_kept() {
    ...
    let row = kf_abi::oracle::truncated_row(cmd).expect("0x208001b0 is a truncated row");
...
fn no_falcon_the_oracle_names_has_a_register_model_this_port_could_serve() {
    ...
    // ⊘ v3: the register model is the HOST family's (`kf_chip::Family::gsp_model`), sized by
