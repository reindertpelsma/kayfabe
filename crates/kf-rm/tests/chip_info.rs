...
fn identity() -> ChipIdentity {
    // ⊘ v3: the identity is the BOARD's (`BoardFacts`), not `identity_for(chip)`.
    let id = ga106::board();
    ChipIdentity {
        pci_vendor_id: id.pci_vendor_id,
        pci_device_id: id.pci_device_id,
        pci_subsystem_vendor_id: id.pci_subsystem_vendor_id,
        pci_subsystem_id: id.pci_subsystem_id,
        pci_revision: id.pci_revision,
    }
}
...
fn the_reply_states_the_same_identity_as_configuration_space() {
    ...
    // builds configuration space from. ⊘ v3: configuration space is built from `BoardFacts`,
    // the same value the policy is given.
    let cfg = ga106::board();
    let got = encode();
    ...
    assert_eq!(packed_device >> 16, u32::from(cfg.pci_device_id));
    assert_eq!(packed_device & 0xFFFF, u32::from(cfg.pci_vendor_id));
    assert_eq!(packed_sub >> 16, u32::from(cfg.pci_subsystem_id));
    assert_eq!(packed_sub & 0xFFFF, u32::from(cfg.pci_subsystem_vendor_id));
    assert_eq!(revision, u32::from(cfg.pci_revision));
}
...
fn a_declared_params_size_that_is_not_ours_is_refused_rather_than_answered() {
    ...
    assert_eq!(reply.rpc_result, kf_abi::NV_ERR_NOT_SUPPORTED);
...
fn a_serialized_request_is_refused_rather_than_answered_flat() {
    ...
    assert!(kf_abi::rpc_params_are_serialized(flags));
...
fn a_register_base_outside_the_aperture_is_refused() {
    ...
    assert!(u64::from(0x00BB_0000u32) < ga106::board().regs_aperture_len());
}

// ⊘ DROPPED in the v3 port: `the_served_register_base_is_a_window_this_devices_own_counter_lives_in`
// asserted `ga10x::GA106.ptimer` offsets — a register-plane fact of the old chip row, which kf-rm
// no longer holds (the PTIMER window belongs to kf-trap / kf-chip).
