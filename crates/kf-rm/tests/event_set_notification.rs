...
//! a copy ([`kf_abi::eventnotify::DELIVERED_NOTIFIERS`], `RegPlane::announce_completion`),
...
fn the_reply_carries_the_registration_back_because_an_empty_body_would_rearm_notifier_zero() {
    // ★★★ **The whole reason this control is not in `kf_rm::inert`.**
    //
...
fn the_reply_is_re_encoded_and_not_reflected_so_the_pad_runs_come_back_zero() {
    // ★★ The distinction `kf_rm::inert`'s own table draws between "the request,
...
fn probed_policy(csv: &str) -> InitTablePolicy {
    InitTablePolicy::with_probe_arm(
        ...
        *table_for(BENCH_DRIVER).expect("bench ABI"),
        kf_abi::eventnotify::ProbeArmSet::parse(csv).expect("test set parses"),
    )
}
...
fn the_probe_admits_exactly_the_named_index_and_the_default_still_refuses_an_unlisted_one() {
    ...
    // never because it is silent. See `kf_abi::eventnotify::DELIVERED_NOTIFIERS`.
    //
...
fn the_delivered_list_is_exactly_index_35_and_is_disjoint_from_the_silent_one() {
    ...
    for n in DELIVERED_NOTIFIERS {
        ...
        assert!(
            !kf_abi::eventnotify::is_silent_notifier(n.index),
