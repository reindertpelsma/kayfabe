...
//! not something a test can quantify over. `kf_rm::sweep::SWEEP_TRIAGE` turns each
...
use kf_rm::sweep::{SWEEP_TRIAGE, SweepDisposition, must_serve_and_unserved, triage_for};
...
fn the_triage_universe_is_pinned_so_shortening_it_is_a_red_test() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            // different artifact of the same real GA106. See kf_abi::grstatic.
...
fn a_control_whose_refusal_is_invisible_must_cite_the_oracles_own_reply() {
    ...
    // contradicted by hardware (`kf_abi::fmbsize`), so a `C:` citation of an *empty*
...
fn a_citation_of_an_uncaptured_oracle_row_must_carry_what_hardware_said() {
    ...
    // mechanically decidable: `kf_abi::oracle::EMPTY_CAPTURE_ROWS` enumerates every
    ...
    for c in SWEEP_TRIAGE {
        let Some(row) = kf_abi::oracle::empty_capture_row(c.cmd) else {
            continue;
        };
        ...
        if kf_abi::oracle::captured_row_evidence(row.cmd, row.psize, 0).is_ok() {
            continue;
        }
        checked += 1;
...
fn a_halting_refusal_may_be_served_or_not_and_the_table_says_which() {
    ...
    // `kf_abi::eventnotify::SILENT_NOTIFIERS`, which scopes the promise to exactly the
    ...
    // `kf_abi::l2evict`, which argues the postcondition already holds structurally and
