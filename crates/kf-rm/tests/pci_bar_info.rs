...
/// ⊘ v3: the BAR table is a fact of the BOARD we present (`BoardFacts::pci_bars`), not a chip row.
fn chip() -> kf_rm::BoardFacts {
    ga106::board_at(ga106::FB_SIZE_MB)
}

fn pci_bar_len(b: &kf_rm::BoardFacts, index: usize) -> u64 {
    b.pci_bars.get(index).map_or(0, |r| r.size_bytes)
}
...
fn the_policy_answers_the_control_without_reflecting_one_byte_of_the_request() {
    ...
    // two fresh boots; `kf_abi::pcibars` carries the dmesg).
...
fn the_chip_row_states_the_apertures_this_device_actually_presents() {
    ...
    // ⊘ v3: there is no separate `identity_for` statement any more — the shell realizes
    // against the SAME `BoardFacts::pci_bars` the guest is told, and the register aperture's
    // length is read from that table, so there is one number per aperture.
    let id = chip();
    assert_eq!(pci_bar_len(&id, bus_bar::FB), 256 * 1024 * 1024);
    assert_eq!(pci_bar_len(&id, bus_bar::INST), 32 * 1024 * 1024);
...
fn the_ga106_rows_are_reachable_by_name_and_are_the_chips_own() {
    ...
    assert_eq!(ga106::pci_bars().len(), chip().pci_bars.len());
    assert_eq!(
        ga106::pci_bars()[bus_bar::FB].size_bytes,
        pci_bar_len(&chip(), bus_bar::FB)
    );
    // An index the chip does not declare reads zero — the protocol's own "not present".
    assert_eq!(pci_bar_len(&chip(), 7), 0);
}
