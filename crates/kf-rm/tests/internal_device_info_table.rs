...
//! in the fixture's `ga106::ENGINES` (old `kayfabe_device::ga10x::GA106_ENGINES`), one layer up, and it decides against six of
//! the oracle's twelve.
...
fn every_oracle_byte_this_file_reads_is_inside_what_the_recorder_kept() {
    ...
    let row = kf_abi::oracle::truncated_row(cmd).expect("0x20800a40 is a truncated row");
...
/// The fixture's GA106 device-info row (old `ga10x::GA106_DEVICE_INFO`).
fn device_info() -> DeviceInfoRow {
    chip().device_info
}
...
/// to `HostFacts` does not have to be restated here with a value invented for it. The two
...
fn bad_chip(
    ...
    std::sync::Arc::new(kf_rm::HostFacts {
