...
//! `kf_abi::fmbsize` with the run that produced it. A test that asserted `20480` while
...
fn a_chip_that_states_no_size_is_refused_at_realize_not_served_a_zero() {
    ...
    //
    // ⊘ v3: there is no `identity_for(chip)` any more — the assembly point is where the fact
    // is DERIVED from the host's reply (`hostfacts::derive_ce_fault_method_buffer_size`), and
    // that is where a zero must be refused, by name. (The old `ChipError` text naming
    // `0x25:0x1f:1249` went with `identity_for`; the refusal now names the control and why.)
    let refused = hostfacts::derive_ce_fault_method_buffer_size(&0u32.to_le_bytes());
    let Err(FactRefusal::Unservable { cmd, why }) = refused else {
        panic!("a host that states no size must not realize: {refused:?}");
    };
    assert_eq!(cmd, NV2080_CTRL_CMD_CE_GET_FAULT_METHOD_BUFFER_SIZE);
