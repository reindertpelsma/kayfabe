...
use kf_abi::capability::{ControlPermit, PassthroughRule, RM_GSS_LEGACY_MASK};
use kf_abi::versions::{BENCH_DRIVER, DriverAbiTable, table_for};
use kf_abi::{GuestOs, NV_ERR_NOT_SUPPORTED};
use kf_arch::ids::ControlCmd;
use kf_gsp::{CommandPolicy, RpcCommand, RpcFunction};
use kf_rm::rmrpc::{BridgeRefusal, Faulted, GraphObjects, GraphPolicy, Translation, translate};
...
/// ⊘ v3: the object model behind the policy is the host-free [`GraphObjects`] (the graph
/// alone) — nothing here reaches the object model, which is the point of every test below.
fn fresh_gpu() -> GraphObjects {
    GraphObjects::new(kf_chip::Family::Ampere)
}
...
fn an_accepted_answer_echoes_the_guests_own_cacheability_bits_verbatim() {
    ...
    let accepted = kf_gsp::RpcCommand {
