...
fn every_oracle_byte_this_file_reads_is_inside_what_the_recorder_kept() {
    ...
    let row = kf_abi::oracle::truncated_row(cmd).expect("0x20801112 is a truncated row");
    let r = kf_abi::oracle::capture_reliance(cmd).expect("and it carries a reliance");
    assert_eq!(
        r.read_end, ORACLE_DEEPEST_BYTE,
        "this file and kf_abi::oracle must agree on how deep the argument reaches"
    );
    assert!(
        kf_abi::oracle::field_is_captured(0, ORACLE_DEEPEST_BYTE, row.kept),
...
fn policy() -> InitTablePolicy {
    let abi = kf_rm::abi::gsp_abi_for(kf_abi::versions::BENCH_DRIVER)
        .expect("the bench driver has a wire table");
    InitTablePolicy::new(ga106::board(), ga106::host(), abi.driver)
}
...
fn a_size_this_port_does_not_encode_is_refused_loudly_not_answered() {
    ...
    assert_eq!(r.rpc_result, kf_abi::NV_ERR_NOT_SUPPORTED);
...
fn every_variant_of_the_served_universe_round_trips_through_its_own_control_id() {
    ...
    // read out of a tree or a capture — see `kf_abi::fmbsize`.
    ...
    // hardware CORROBORATED it byte for byte — see `kf_abi::grinfo`.
    ...
    // never reach a GSP. See `kf_abi::gpuinfo`.
    ...
    // seed was the invalid captype. See `kf_abi::gpuatomics`.
    ...
    // boot, so the absence meant nothing. See `kf_abi::fbinfo`.
    ...
    // `portMemSet`s the probe's own seed away before forwarding. See `kf_abi::cecaps`.
    ...
    // inside an `NV_OK` reply, which reaches neither ledger. `kf_abi::grfsinfo` refuses
    ...
    // `580.159.04` — the reading §14.35's own wording invited. See `kf_abi::gspfeatures`.
    ...
    // `SKIP_COPYOUT` to hide behind — see `kf_abi::gsslegacy`, which also carries why
    ...
    // `kf_abi::faultbuffer`'s module docs, and the unbuilt half is printed in every
    ...
    // ★ 52 -> 53 (2026-09-26, `V3_FAMILY_PORT_BLACKWELL.md` §4): `0x20800a34`
    // INTERNAL_STATIC_KGR_GET_SM_ISSUE_RATE_MODIFIER, from the host's unprivileged
    // GR_GET_SM_ISSUE_RATE_MODIFIER — libcuda's `cuInit` asks the client control on Blackwell.
    assert_eq!(WantedTable::ALL.len(), 53, "the served universe's size");
...
fn no_control_this_port_serves_can_be_cached_permanently_by_the_guest() {
    ...
    // that decision, and the reasoning is in `kf_abi::gsslegacy`. So the property
    ...
    // convenient. `kf_abi::cudartinit` carries the argument for its three, and it is a
    ...
    let argument_bearing: std::collections::BTreeSet<u32> = gss_legacy_served
        .iter()
        .copied()
        .filter(|id| kf_abi::gsslegacy::carries_cache_argument(*id))
        .collect();
    assert_eq!(
    ...
    for (cmd, size, _) in kf_abi::cudartinit::SERVED {
        let a = kf_abi::cudartinit::answer_cudart_init(*cmd, *size).expect("served");
        let b = kf_abi::cudartinit::answer_cudart_init(*cmd, *size).expect("served");
        assert_eq!(
            a, b,
    ...
    let probe: Vec<u8> = (0..kf_abi::gsslegacy::GSS_LEGACY_0X8159_PARAMS_SIZE)
    ...
    assert_ne!(
        kf_abi::gsslegacy::answer_gss_legacy(kf_abi::gsslegacy::GSS_LEGACY_0X8162, &[0]),
        Ok(vec![0u8]),
    ...
    assert!(kf_rm::inittables::is_gss_legacy(0x2080_8513));
    assert!(!kf_rm::inittables::is_gss_legacy(
        NV2080_CTRL_CMD_FIFO_GET_DEVICE_INFO_TABLE
    ));
    assert!(kf_rm::inittables::is_gss_legacy(
