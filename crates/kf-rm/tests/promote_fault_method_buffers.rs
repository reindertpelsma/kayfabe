...
//! with the chip flag **negated**, `kf_abi::c2cinfo`'s own unit tests stayed green
...
//! the wire.
//!
//! `kf_abi::fmbpromote`'s tests are exactly that kind of callee test. This file asserts
...
/// `kf_abi::fmbpromote`'s encoder, so a wrong offset there cannot make this file agree
/// with it.
...
/// `kf_abi::fmbsize` answers this same guest — in `ADDR_SYSMEM`, `bar2Addr = 0`.
fn realistic() -> Vec<u8> {
    params(
        &[
