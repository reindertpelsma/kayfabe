...
pub struct ChannelAllocFacts {
    ...
    /// `kf_arch::PushRange`'s VA-vs-GPA question `[unmeasured]`. See
    ...
    /// the channel's `VChid` from it (`kf_arch::Arch::vchid_from_userd_flags`),
...
impl PdbAperture {
    ...
    #[must_use]
    pub fn to_domain(self) -> Option<kf_arch::Aperture> {
        match self {
            Self::Vidmem => Some(kf_arch::Aperture::Vidmem),
            Self::SysmemCoherent => Some(kf_arch::Aperture::SysmemCoherent),
            Self::SysmemNoncoherent => Some(kf_arch::Aperture::SysmemNonCoherent),
...
pub fn classify_promote_entry(
    ...
    let aperture = |phys_attr: u32| -> Result<kf_arch::Aperture, AbiError> {
        match phys_attr & 0x3 {
            0 => Ok(kf_arch::Aperture::Vidmem),
            1 => Ok(kf_arch::Aperture::SysmemCoherent),
            2 => Ok(kf_arch::Aperture::SysmemNonCoherent),
...
mod the_pdb_aperture_must_not_fold_into_a_default {
    ...
    fn each_defined_aperture_maps_to_its_own_domain_value() {
        assert_eq!(
            ...
            Some(kf_arch::Aperture::Vidmem)
        );
        assert_eq!(
            PdbAperture::SysmemCoherent.to_domain(),
            Some(kf_arch::Aperture::SysmemCoherent)
        );
        assert_eq!(
            PdbAperture::SysmemNoncoherent.to_domain(),
            Some(kf_arch::Aperture::SysmemNonCoherent)
        );
    }
    ...
    fn an_undefined_aperture_is_unknown_and_never_vidmem() {
        for raw in [3u32, 7, 0xffff_ffff] {
            ...
            assert_ne!(a.to_domain(), Some(kf_arch::Aperture::Vidmem));
        }
    }
