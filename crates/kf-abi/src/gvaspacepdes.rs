...
//! `kf_abi::l2evict`'s header enumerates, and it must be re-decided there rather than
//! inherited.
//!
...
#[must_use]
pub fn decode_aperture(raw: u32) -> Option<kf_arch::Aperture> {
    match raw {
        GMMU_APERTURE_VIDEO => Some(kf_arch::Aperture::Vidmem),
        GMMU_APERTURE_PEER => Some(kf_arch::Aperture::Peer),
        GMMU_APERTURE_SYS_COH => Some(kf_arch::Aperture::SysmemCoherent),
        GMMU_APERTURE_SYS_NONCOH => Some(kf_arch::Aperture::SysmemNonCoherent),
