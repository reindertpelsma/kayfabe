...
/// `kf_gsp::GspFsm::answer` posts when *nobody claimed the command*, and it is the
...
/// refusal here: `0x56` is `kf_gsp::GspFsm::answer`'s signature for *"nobody claimed
...
/// `0x56` is `kf_gsp::GspFsm::answer`'s signature for *"nobody claimed this command"*,
...
/// *are* is the [`Arch`](../../kf_arch/trait.Arch.html)'s question, one crate up.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MethodForm {
...
/// dword would report a class id nothing in `kf_abi::generated::classes` names,
...
pub mod ce {
    ...
    /// [`kf_arch::MethodState::subchannel_speaks`], which additionally requires that
