...
///
/// ★★★ Read from [`kf_abi::oracle`]'s census rather than written as `176`. The literal
...
fn oracle() -> Vec<u8> {
    ...
    assert!(
        kf_abi::oracle::field_is_captured(0, b.len(), kept),
    ...
    assert!(
        !kf_abi::oracle::field_is_captured(0, COPY_SERVER_RESERVED_PDES_PARAMS_SIZE, kept),
...
/// reliance statement in `kf_abi::oracle` says the same thing, and the two must agree.
#[test]
fn every_oracle_byte_this_file_reads_is_inside_what_the_recorder_kept() {
    ...
    let r = kf_abi::oracle::capture_reliance(cmd).expect("0x20800a9f carries a reliance");
    assert_eq!(
        r.read_end,
        oracle_captured_len(),
