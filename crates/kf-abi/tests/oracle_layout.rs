...
fn rpc_constants_match_the_c_emulators_literals() {
    assert_eq!(
        kf_abi::view::RpcEnvelope::SIGNATURE_VALID,
        0x4350_5256,
...
fn the_channel_alloc_prefix_stops_where_the_two_trees_stop_agreeing() {
    use kf_abi::versions::CHANNEL_ALLOC_PREFIX;
    assert_eq!(
    ...
    assert_eq!(
        ...
        Err(kf_abi::wire::AbiError::Truncated {
...
///
/// That is why [`kf_abi::versions::DriverAbiTable::decode_client_alloc_facts`]
...
fn nv0000_alloc_params_has_only_its_prefix_corroborated() {
    ...
    assert_eq!(kf_abi::versions::CLIENT_ALLOC_PREFIX, 8);
...
fn the_gsp_element_wire_boundary_is_610_not_570() {
    ...
    use kf_abi::versions::{BENCH_DRIVER, GspElementWire, GspInitArgsWire, table_for};
...
fn the_610_transport_words_are_assembled_from_the_drivers_own_bit_fields() {
    use kf_abi::versions::GspElementWire;
...
fn the_bench_driver_declares_no_queue_geometry_and_the_table_supplies_it() {
    use kf_abi::versions::{BENCH_DRIVER, GspInitArgsWire, table_for};
    ...
    let new = table_for(kf_abi::DriverVersion {
        major: 610,
        minor: 43,
        patch: 2,
    })
