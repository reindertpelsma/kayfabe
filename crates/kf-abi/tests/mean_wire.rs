...
use kf_abi::generated::{classes, ctrl, nvos, rpc};
use kf_abi::transcribed::Nvos46ParametersPre580;
use kf_abi::versions::{BENCH_DRIVER, DriverAbiTable, MapDmaWire, table_for};
use kf_abi::view::{AllocWire, PdbAperture, RpcEnvelope};
use kf_abi::wire::AbiError;
use kf_abi::{DriverAbi, DriverVersion, GuestOs};
...
fn a_realistic_event_stream_decodes_and_the_skewed_event_in_the_middle_is_refused() {
    ...
    assert_eq!(
        t.alloc_param_size(kf_arch::ids::ClassId(a.class)),
        None,
    ...
    assert_eq!(
        t.alloc_param_size(kf_arch::ids::ClassId(classes::NV01_DEVICE_0)),
        Some(56)
    );
...
fn the_control_table_names_exactly_the_commands_it_can_argue_for() {
    use kf_abi::versions::ControlParams;
    ...
    assert_eq!(
        ...
        Some(kf_abi::gvaspacepdes::COPY_SERVER_RESERVED_PDES_PARAMS_SIZE),
    );
...
fn the_copy_engine_inverse_is_exact_across_both_blocks_and_the_gap_between_them() {
    use kf_abi::submit::{ENGINE_TYPE_COPY0, ENGINE_TYPE_COPY10, copy_index_of_engine_type};
    ...
    for i in 0..10u32 {
        let wire = kf_abi::submit::engine_type_copy(i).expect("first block");
