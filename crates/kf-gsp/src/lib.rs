...
//!    they are behind `kf_arch::GspModel`, reached through `Arch::gsp()`. The suite
...
//!    `kf_abi::versions::GspElementWire`, and the wire tables are keyed on the full
//!    `major.minor.patch` with a loud [`kf_abi::wire::AbiError::NoTableForVersion`]
...
pub use boot::{
    CommandObserver, CommandPolicy, Deferred, EchoOk, EventDelivery, GspAbi, GspFsm, InitArgsLayout,
    Observing, PolicyChain, QueueBinding, QueueState, Reply, ServiceReport, Transition, Unserviced,
    RM_ALLOC_PARAMS_STATUS_AT, stamp_alloc_status,
};
...
// `kf_gsp::BootPhase` path still resolves.
pub use element::{
