...
pub enum GspFault {
    /// The architecture has no [`kf_arch::GspModel`]. MISS = FAULT: there is no
    ...
    /// This generation's [`kf_arch::BootSequence`] returned more steps for one write
    /// than [`kf_arch::BootSteps::CAPACITY`] holds.
    ///
...
impl From<kf_abi::wire::AbiError> for GspFault {
    fn from(e: kf_abi::wire::AbiError) -> Self {
        GspFault::Envelope(e)
    }
}

kf_util::assert_send_sync!(GspFault, RxLinkCode, RegionError, LayoutError, RamRefused);
