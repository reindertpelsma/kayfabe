...
impl MappedRegion {
    ...
    ///
    /// # Errors
    /// [`RawError::ZeroLength`], [`RawError::Misaligned`] (length, or a file offset),
    /// [`RawError::TooLargeForHost`], [`RawError::Syscall`].
    ///
    /// # Panics
...
impl VolatileRegion {
    /// Map `len` bytes of `backing` as a hardware-shared region. Always read-write: a page
    /// whose *other* writer is a GPU is not usefully read-only to us.
    ///
    /// # Errors
    /// As [`MappedRegion::map`].
    ///
    /// # Panics
    /// If called with any ranked lock held (R1, §4.5).
    pub fn map(backing: Backing<'_>, len: u64, page: HostPageSize) -> Result<Self, RawError> {
...
mod tests {
    ...
    fn a_mapped_region_round_trips_through_copies_only() {
        let p = page();
        let r = MappedRegion::map(Backing::PrivateAnonymous, p.bytes(), HostProt::ReadWrite, p)
            .expect("map");
    ...
    fn writing_a_read_only_region_is_refused_before_the_store() {
        let p = page();
        let r = MappedRegion::map(Backing::PrivateAnonymous, p.bytes(), HostProt::ReadOnly, p)
            .expect("map");
        assert_eq!(
    ...
    fn a_mapping_length_must_be_whole_host_pages_and_non_zero() {
        let p = page();
        assert_eq!(
            MappedRegion::map(Backing::PrivateAnonymous, 0, HostProt::ReadWrite, p).unwrap_err(),
            RawError::ZeroLength {
                what: "mapping length"
            }
        );
    ...
    fn volatile_words_load_and_store_at_both_widths() {
        let p = page();
        let v = VolatileRegion::map(Backing::PrivateAnonymous, p.bytes(), p).expect("map");
    ...
    fn mapping_under_a_held_lock_violates_r1() {
        ...
        let outcome = std::panic::catch_unwind(|| {
            MappedRegion::map(Backing::PrivateAnonymous, p.bytes(), HostProt::ReadWrite, p)
        });
