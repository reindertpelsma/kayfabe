...
/// (decision #2), only in `kayfabe-abi` (`kf_abi::client_kind_from_process_id`).
...
/// (`kf_abi::bringup::NV_VASPACE_ALLOCATION_INDEX_GPU_DEVICE`, which also carries
...
/// `kf_abi::notifier::ChannelUserdMemWire`, which carries the driver citation and the
...
pub enum FbWindow {
    ...
    Pramin,
    /// Base-address register `kf_abi::pcibars::bus_bar::FB` — the framebuffer
    /// aperture. GMMU-translated through the `bar1PdeBase` this port itself reports
    /// (`C: :4604`).
    FbAperture,
    /// Base-address register `kf_abi::pcibars::bus_bar::INST` — RM's `BAR2` window.
...
/// ```compile_fail
/// let va = kf_arch::ids::GpuVa(0x4_2000_0000);
/// let here: kf_arch::PlaneAddr = va;
...
/// ```
/// let va = kf_arch::ids::GpuVa(0x4_2000_0000);
/// let here = kf_arch::PlaneAddr(va.0);
/// ```
///
...
/// ```compile_fail
/// let here = kf_arch::PlaneAddr(0x2f2c_b004);
/// let raw: u64 = here;
/// // error[E0308]: mismatched types
...
pub enum PushMethod {
    ...
    /// doorbells. See `kf_abi::submit::uvm_sw` for the predicate and its refutation
...
pub struct CeCompletion {
    ...
    /// `kf_abi::submit::ce::SET_SEMAPHORE_PAYLOAD_UPPER` is not consulted at all;
...
pub enum CeSemStructure {
    ...
    /// [`kf_abi::submit::ce::SEM_FOUR_WORD_TIMESTAMP_OFFSET`].
    ///
...
/// `kf_abi::submit::gp_entry_decode` names its own field `gpu_va` for that reason.
///
...
pub trait PushbufferAbi: Send + Sync {
    ...
    /// comes from `kf_abi`, because indexing the ring needs a byte offset and no
    /// `PushbufferAbi` method hands one out"* — and reached around the trait to
    /// `kf_abi::submit::GP_ENTRY_SIZE` to get it, which is a GA10x constant used as if
    /// it were universal.
    ///
