...
fn the_pramin_window_and_its_base_register_are_each_die_groups_header() {
    ...
    for g in FALCON_ERA {
        ...
        assert_eq!(
            u64::from(r.base_mask),
            mask(g, "NV_PBUS_BAR0_WINDOW_BASE"),
            "{g:?}"
        );
        assert!(
            r.has_target && range(g, "NV_PBUS_BAR0_WINDOW_TARGET") == (25, 24),
            "{g:?}"
        );
        assert_eq!(
            u64::from(pramin::BASE_SHIFT),
            val(g, "NV_PBUS_BAR0_WINDOW_BASE_SHIFT"),
            "{g:?}"
        );
    }
    for g in FSP_ERA {
        ...
        assert_eq!(
            u64::from(pramin::BASE_SHIFT),
            val(g, "NV_XAL_EP_BAR0_WINDOW_BASE_SHIFT"),
            "{g:?}"
        );
    }
    ...
    assert_eq!(
        u64::from(gh.base_mask),
        mask(DieGroup::Gh100, "NV_XAL_EP_BAR0_WINDOW_BASE")
    );
    ...
    assert_eq!(
        bw & mask(DieGroup::Gb10x, "NV_XAL_EP_BAR0_WINDOW_BASE"),
        mask(DieGroup::Gb10x, "NV_XAL_EP_BAR0_WINDOW_BASE")
    );
    assert!(
        table()
            .resolve(DieGroup::Gb20x, "NV_XAL_EP_BAR0_WINDOW_BASE")
            .value()
            .is_none()
    );
    ...
    assert_eq!(
        gb10b,
        Some(kf_chip::hwref::HwValue::Range { hi: 24, lo: 0 }),
        "the width the decode takes"
    );
}
...
fn the_mmu_invalidate_registers_and_fields_are_each_die_groups_header() {
    for g in DieGroup::ALL {
        assert_eq!(
            mmuinval::USERMODE_ABOVE_PRIV,
            base(g, "NV_VIRTUAL_FUNCTION"),
            "{g:?}"
        );
        assert_eq!(
            mmuinval::MMU_INVALIDATE_PDB_OFF,
            val(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE_PDB")
        );
        assert_eq!(
            mmuinval::MMU_INVALIDATE_UPPER_PDB_OFF,
            val(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE_UPPER_PDB")
        );
        assert_eq!(
            mmuinval::MMU_INVALIDATE_OFF,
            val(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE")
        );
        assert_eq!(
            u64::from(mmuinval::PDB_ADDR_ALIGNMENT),
            val(
                g,
        ...
        assert_eq!(
            u64::from(mmuinval::TRIGGER_BIT),
            bit(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE_TRIGGER")
        );
        // The decode's field positions (`Invalidate::decode` / `encode_pdb`).
        assert_eq!(
            range(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE_ALL_VA"),
            (0, 0)
        );
        assert_eq!(
            range(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE_ALL_PDB"),
            (1, 1)
        );
        assert_eq!(
            range(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE_HUBTLB_ONLY"),
            (2, 2)
        );
        assert_eq!(
            range(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE_REPLAY"),
            (5, 3)
        );
        assert_eq!(
            range(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE_PDB_ADDR"),
            (31, 4)
        );
        assert_eq!(
            range(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE_PDB_APERTURE"),
            (1, 1)
        );
        assert_eq!(
            range(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE_UPPER_PDB_ADDR"),
            (19, 0)
        );
        let r =
            mmuinval::InvalidateRegs::from_usermode_base(vf(g) + base(g, "NV_VIRTUAL_FUNCTION"))
                .unwrap();
        assert_eq!(
            r.trigger,
            vf(g) + val(g, "NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE"),
            "{g:?}"
        );
    }
}
...
fn the_cache_op_registers_are_each_die_groups_header() {
    for g in FALCON_ERA {
        ...
        let want = [
            (
                val(g, "NV_UFLUSH_L2_FLUSH_DIRTY"),
                cacheop::CacheOp::FlushDirty,
            ),
            (
                vf(g) + val(g, "NV_VIRTUAL_FUNCTION_PRIV_L2_SYSMEM_INVALIDATE"),
                cacheop::CacheOp::SysmemInvalidate,
            ),
            (
                vf(g) + val(g, "NV_VIRTUAL_FUNCTION_PRIV_L2_PEERMEM_INVALIDATE"),
                cacheop::CacheOp::PeermemInvalidate,
            ),
        ];
        let got: Vec<(u64, cacheop::CacheOp)> =
            regs.iter().map(|(o, op)| (u64::from(*o), *op)).collect();
        assert_eq!(got, want, "{g:?}");
        assert_eq!(
            u64::from(cacheop::PENDING),
            bit(g, "NV_UFLUSH_L2_FLUSH_DIRTY_PENDING"),
            "{g:?}"
        );
        assert!(
            cacheop::token_registers(g.family()).is_empty(),
            "{g:?}: no token protocol before Hopper"
        );
    }
    for g in FSP_ERA {
        ...
        let want = [
            (
                "NV_XAL_EP_UFLUSH_L2_FLUSH_DIRTY",
                0,
                cacheop::CacheOp::FlushDirty,
            ),
            (
                "NV_VIRTUAL_FUNCTION_PRIV_FUNC_L2_SYSMEM_INVALIDATE",
                vf(g),
                cacheop::CacheOp::SysmemInvalidate,
            ),
            (
                "NV_VIRTUAL_FUNCTION_PRIV_FUNC_L2_PEERMEM_INVALIDATE",
                vf(g),
                cacheop::CacheOp::PeermemInvalidate,
            ),
        ...
        for (t, (name, at, op)) in got.iter().zip(want) {
            ...
            assert_eq!(
                u64::from(t.completed),
                at + val(g, &format!("{name}_COMPLETED")),
                "{g:?} {name}_COMPLETED"
            );
            assert_eq!(t.op, op);
            assert_eq!(
                u64::from(cacheop::TOKEN_MASK),
                mask(g, &format!("{name}_COMPLETED_TOKEN")),
                "{g:?} {name}"
            );
            assert_eq!(
                u64::from(cacheop::COMPLETED_BUSY),
                bit(g, &format!("{name}_COMPLETED_STATUS"))
                    * val(g, &format!("{name}_COMPLETED_STATUS_BUSY")),
                "{g:?} {name}"
            );
        }
        assert_eq!(
            u64::from(cacheop::MEMOP_MAX_OUTSTANDING),
            val(g, "NV_XAL_EP_MEMOP_MAX_OUTSTANDING"),
            "{g:?}"
        );
    }
}
...
fn the_cpu_interrupt_tree_is_each_die_groups_header() {
    for g in DieGroup::ALL {
        for (ours, name) in [
            (
                cpuintr::LEAF_OFF,
                "NV_VIRTUAL_FUNCTION_PRIV_CPU_INTR_LEAF(0)",
            ),
            (
                cpuintr::LEAF_EN_SET_OFF,
                "NV_VIRTUAL_FUNCTION_PRIV_CPU_INTR_LEAF_EN_SET(0)",
            ),
            (
                cpuintr::LEAF_EN_CLEAR_OFF,
                "NV_VIRTUAL_FUNCTION_PRIV_CPU_INTR_LEAF_EN_CLEAR(0)",
            ),
            (cpuintr::TOP_OFF, "NV_VIRTUAL_FUNCTION_PRIV_CPU_INTR_TOP(0)"),
            (
                cpuintr::TOP_EN_SET_OFF,
                "NV_VIRTUAL_FUNCTION_PRIV_CPU_INTR_TOP_EN_SET(0)",
            ),
            (
                cpuintr::TOP_EN_CLEAR_OFF,
                "NV_VIRTUAL_FUNCTION_PRIV_CPU_INTR_TOP_EN_CLEAR(0)",
            ),
            (
                cpuintr::LEAF_TRIGGER_OFF,
                "NV_VIRTUAL_FUNCTION_PRIV_CPU_INTR_LEAF_TRIGGER",
            ),
        ] {
            assert_eq!(ours, val(g, name), "{g:?} {name}");
        }
        assert_eq!(
            val(g, "NV_VIRTUAL_FUNCTION_PRIV_CPU_INTR_LEAF(1)")
                - val(g, "NV_VIRTUAL_FUNCTION_PRIV_CPU_INTR_LEAF(0)"),
            4,
            "{g:?}: stride"
        );
        ...
        assert_eq!(
            leaves as u64,
            val(g, "NV_VIRTUAL_FUNCTION_PRIV_CPU_INTR_LEAF__SIZE_1"),
            "{g:?}"
        );
        assert!(leaves <= cpuintr::MAX_LEAF);
        assert_eq!(
            val(g, "NV_VIRTUAL_FUNCTION_PRIV_CPU_INTR_TOP__SIZE_1"),
            1,
            "{g:?}"
        );
        assert_eq!(
            u64::from(cpuintr::DOORBELL_VECTOR),
            val(g, "NV_CTRL_CPU_DOORBELL_VECTORID_VALUE_CONSTANT"),
            "{g:?}"
        );
    }
}
...
fn the_timer_registers_refused_by_name_are_each_die_groups_header() {
    for g in DieGroup::ALL {
        assert_eq!(
            u64::from(timer::VF_TIME_0),
            val(g, "NV_VIRTUAL_FUNCTION_TIME_0"),
            "{g:?}"
        );
        assert_eq!(
            u64::from(timer::VF_TIME_1),
            val(g, "NV_VIRTUAL_FUNCTION_TIME_1"),
            "{g:?}"
        );
    }
    for g in FALCON_ERA {
        let t = timer::timer_regs_for(g.family());
        let want: Vec<u32> = ["NV_PTIMER_TIME_0", "NV_PTIMER_TIME_1"]
            .iter()
            .map(|n| val(g, n) as u32)
            .collect();
        assert_eq!(t.refused_writes, want.as_slice(), "{g:?}");
        assert_eq!(
            t.priv_level_mask.map(u64::from),
            Some(val(g, "NV_PTIMER_TIME_PRIV_LEVEL_MASK")),
            "{g:?}"
        );
        assert_eq!(
            ...
            bit(g, "NV_PTIMER_TIME_PRIV_LEVEL_MASK_WRITE_PROTECTION_LEVEL0")
                * val(
                    g,
                    "NV_PTIMER_TIME_PRIV_LEVEL_MASK_WRITE_PROTECTION_LEVEL0_ENABLE"
                ),
        );
    }
    for g in FSP_ERA {
        let t = timer::timer_regs_for(g.family());
        let want: Vec<u32> = [
            "NV_PGC6_SCI_SYS_TIMER_OFFSET_0",
            "NV_PGC6_SCI_SYS_TIMER_OFFSET_1",
        ]
        .iter()
        .map(|n| val(g, n) as u32)
        .collect();
...
fn the_memory_map_pages_are_each_die_groups_header() {
    for g in DieGroup::ALL {
        assert_eq!(
            memmap::VF_USERMODE_PAGE,
            vf(g) + base(g, "NV_VIRTUAL_FUNCTION"),
            "{g:?}"
        );
    }
    ...
    for g in FSP_ERA {
        let holes: Vec<u64> = memmap::holes_for(g.family())
            .iter()
            .map(|(p, _)| *p)
            .collect();
        for need in [
            ...
            assert!(
                holes.contains(&need),
                "{g:?}: page {need:#x} must be a hole"
            );
        }
    }
    // The integrated-Blackwell SEC2 EMEM page — outside every discrete lineage, from GB10B's header.
    let Some(kf_chip::hwref::HwValue::Val(sec2)) =
        table().in_dir("blackwell/gb10b", "NV_PSEC_EMEMD(0)")
    else {
        panic!("gb10b publishes NV_PSEC_EMEMD");
    };
    assert!(
        memmap::holes_for(kf_chip::Family::Blackwell)
            .iter()
            .any(|(p, _)| *p == page(sec2))
    );
}
