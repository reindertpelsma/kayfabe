...
/// ★★★★★ **w755c — DID THE RESERVATION COME BACK CONTIGUOUS AND 1 GiB-ALIGNED?**
///
/// ⊘ Set by [`RmConnection::reserve_gpga`], read by the store-slice page-size decision.
/// **Recorded rather than re-derived**: the two are one decision, and a map site that
/// guessed the reservation's shape would be a second statement of it.
///
/// ⚠ There is exactly one reserved store per VMM process, which is why a process-wide flag
/// is the honest shape here rather than a per-connection field. If that ever stops being
    ...
    impl BirthConn {
        ...
        pub(super) fn map_dma_slice(
            ...
            let page_size = kayfabe_abi::bringup::nvos46_page_size_flag_for_store_slice(
                super::STORE_IS_CONTIGUOUS_AND_ALIGNED.load(std::sync::atomic::Ordering::Relaxed),
            );
            Nvos46Parameters {
                h_client: self.handed.root(),
...
impl RmConnection {
    ...
    fn reserve_gpga_inner(&self, len: u64, is_the_real_store: bool) -> Result<u32, RmError> {
        ...
        NvMemoryAllocationParams {
            ...
            kind: 0,
            attr: kayfabe_abi::submit::ATTR_CONTIGUOUS_VIDMEM,
            size: len,
            alignment: ONE_GIB,
        }
