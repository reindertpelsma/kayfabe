...
/// assertion in the area (`userd_fits_inside_the_ring_object_without_touching_the_other_three_regions`)
/// checks that *our own constant* is aligned, which is the opposite direction: it exercises the
/// value we control and never the value the **guest** supplies. A rule reachable only through a
/// GPU is a rule that is reviewed rather than tested, and this one refuses guest input.
///
...
mod tests {
    ...
    ///
    /// ★ The three facts are asserted **separately**, in both directions, so a future
    /// re-collapse of the conjunction cannot pass: each row below removes exactly one fact from
    /// an otherwise-complete evidence value and requires the verdict to move.
    ///
