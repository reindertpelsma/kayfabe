...
/// ★★★ **What `NV_ESC_RM_DUP_OBJECT` answered, as RM said it** — the B1(d) probe's
/// result type (`docs/design/SINGLE_STORE_PLAN.md`, the `(d)` ruling).
///
/// ⊘ **Why not `Result<u32, RmError>`.** [`status_check`] maps `0x1A` and `0x51` onto one
/// [`RmError::NoMemory`] arm, and that collapse is correct for production — a caller that
/// must give up does not care which — and **wrong for a measurement**. The C treats `0x51`
/// on a FIXED map as *"the VA is ALREADY mapped"*, i.e. a positive; `0x1A` is a genuine
/// capacity refusal. One enum arm, two opposite findings. A probe reports the driver's own
/// number or it cannot be cited.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct DupOutcome {
    /// RM's `NVOS55_PARAMETERS::status`, verbatim. `0` is `NV_OK`.
    pub status: u32,
    /// `NVOS55_PARAMETERS::hObject` as written back. Meaningless unless `status == 0`.
    pub h_object: u32,
}

/// What `NV_ESC_RM_ALLOC` answered, as RM said it. See [`DupOutcome`] for why the status
/// is carried rather than classified.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AllocOutcome {
    /// RM's `NVOS21_PARAMETERS::status`, verbatim.
    pub status: u32,
    /// `NVOS21_PARAMETERS::hObjectNew` as written back.
    pub h_object: u32,
}

/// What `NV_ESC_RM_MAP_MEMORY_DMA` answered, as RM said it.
///
/// ★ `dma_offset` is carried **even when `status != 0`**, because the interesting failure
/// is not *"refused"* but *"accepted and placed somewhere I did not ask for"* — and with
/// `DMA_OFFSET_FIXED_TRUE` that second case is supposed to be impossible. A probe that
/// dropped the field on refusal could not tell the two apart.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MapOutcome {
    /// RM's `NVOS46_PARAMETERS::status`, verbatim.
    pub status: u32,
    /// `NVOS46_PARAMETERS::dmaOffset` as written back — the VA RM says it used.
    pub dma_offset: u64,
}
...
impl RmConnection {
    ...
    /// ★★★ **B1(d) PROBE — one `NV_ESC_RM_DUP_OBJECT`, with RM's own status returned
    /// rather than classified.**
    ///
    /// Duplicates `src_object`, which lives under client `src_client`, into **this**
    /// connection's client, parented at `parent`. The design ruled in
    /// `docs/design/SINGLE_STORE_PLAN.md` §*"(d) NOBODY BUT THE SCRATCHPAD EVER NAMES THE
    /// OBJECT"* rests on this call accepting an address-space class, which
    /// `NV_ESC_RM_DUP_OBJECT` is **not** documented to support for every class — hence a
    /// probe rather than a production verb.
    ///
    /// ⊘ **It returns [`DupOutcome`], not `Result<u32>`, and that is the point.**
    /// [`status_check`] deliberately collapses `0x1A`/`0x51` into one
    /// [`RmError::NoMemory`] and `0x1B` into [`RmError::InsufficientPermissions`]. For a
    /// *measurement* that is exactly the wrong shape: `0x51` on a FIXED map means *"the VA
    /// is already mapped"* and is a POSITIVE result for this design, while `0x1A` is a
    /// capacity refusal — two different findings behind one enum arm. A probe that cannot
    /// print the driver's own number cannot be cited.
    ///
    /// # Errors
    /// Only if the **ioctl itself** failed (a syscall-level error, or a struct this crate
    /// could not encode). An RM refusal is **not** an error here — it is the `status`
    /// field of the returned [`DupOutcome`].
    fn raw_dup_object(
        &self,
        parent: u32,
        want: u32,
        ...
        Ok(DupOutcome {
            status: out.status,
            h_object: out.h_object,
        })
    }

    /// ★★ **B1(d) PROBE — an `NV01_MEMORY_VIRTUAL` range over a CALLER-SUPPLIED address
    /// space handle.**
    ///
    /// [`RmConnection::alloc_vaspace_raw`] mints the `FERMI_VASPACE_A` and its range
    /// together and pairs them, which is right for the production path and useless for the
    /// question (d) asks: *can this client build a range over an address space it did not
    /// create — one that was **duped in** from another client?* That needs the space handle
    /// to be a parameter.
    ///
    /// ⊘ The range is remembered under `self.device` but deliberately **not** paired with
    /// `h_va_space`: pairing means *"freeing the range frees the space"*, and a duped-in
    /// space is not this connection's to free.
    ///
    /// # Errors
    /// As [`RmConnection::raw_dup_object`] — ioctl-level only; RM's refusal is `status`.
    fn raw_alloc_virtual_range(&self, h_va_space: u32) -> Result<AllocOutcome, RmError> {
    ...
    fn raw_alloc_status(
        ...
        let out = Nvos21Parameters::decode(&arg).map_err(|_| RmError::Other(NOT_ON_THIS_RUNG))?;
        if out.status == 0 {
            self.remember(out.h_object_new, parent);
        }
        Ok(AllocOutcome {
            status: out.status,
            h_object: out.h_object_new,
        })
    }

    /// ★★★ **B1(d) PROBE — one `NV_ESC_RM_MAP_MEMORY_DMA` at a CALLER-CHOSEN VA, with
    /// RM's status and the `dmaOffset` it actually wrote back.**
    ///
    /// Differs from [`RmConnection::raw_map_dma_flags`] in the two ways the probe needs:
    /// it never classifies the status (see [`RmConnection::raw_dup_object`]), and it
    /// returns `dmaOffset` **even on refusal**, so *"RM refused"* and *"RM placed it
    /// somewhere else"* are distinguishable. `DMA_OFFSET_FIXED_TRUE` is always set — an
    /// unfixed map answers a different question than the one (d) asks.
    ///
    /// # Errors
    /// Ioctl-level only.
    fn raw_map_dma_fixed_status(
        &self,
        h_dma: u32,
        h_memory: u32,
        len: u64,
...
impl HostRmBackend {
    ...
    /// ★★★★★ **B1(d) PROBE — the address-space PAIR, both handles, unfreed.**
    ///
    /// [`HostRmBackend::host_alloc_vaspace_space`] returns only the `FERMI_VASPACE_A` and
    /// the `RmBackend::alloc_vaspace` port returns only the `NV01_MEMORY_VIRTUAL` range.
    /// The B1(d) probe needs **both**, because the two are duped separately and the whole
    /// question is which of them `NV_ESC_RM_DUP_OBJECT` accepts and which one
    /// `NVOS46_PARAMETERS::hDma` will then take.
    ///
    /// Returns `(range, space)` — range first, matching `alloc_vaspace_raw`'s own return.
    ///
    /// # Errors
    /// Propagates RM's refusal; a caller must not substitute a handle.
    pub fn host_alloc_vaspace_pair(&mut self) -> Result<(u32, u32), RmError> {
        let range = self.alloc_vaspace_raw()?;
        let space = self
            .conn
            .space_of(range)
    ...
    /// backend's client.** The hinge the whole (d) ruling rests on.
    ///
    /// Parented at this connection's **device**, which is where both candidate classes
    /// (`FERMI_VASPACE_A` and `NV01_MEMORY_VIRTUAL`) live in their source client — RM
    /// requires the destination parent to be a legal parent for the class, and the device
    /// is the only object this connection has that is one.
    ///
    /// # Errors
    /// Ioctl-level only — RM's refusal is [`DupOutcome::status`].
    pub fn host_dup_object(
        &self,
        src_client: u32,
        src_object: u32,
        flags: u32,
        ...
        let want = self.conn.mint();
        self.conn
            .raw_dup_object(self.conn.device, want, src_client, src_object, flags)
    }

    /// ★★★ **B1(d) PROBE — an `NV01_MEMORY_VIRTUAL` range over `h_va_space`**, which may
    /// be a space this client duped in rather than one it created.
    ///
    /// # Errors
    /// Ioctl-level only — RM's refusal is [`AllocOutcome::status`].
    pub fn host_alloc_virtual_range_over(&self, h_va_space: u32) -> Result<AllocOutcome, RmError> {
        self.conn.raw_alloc_virtual_range(h_va_space)
    }

    /// ★★★ **B1(d) PROBE — reserve non-contiguous device memory**, the `reserve_gpga`
    /// shape, returning the bare handle the probe must name in `NVOS46::hMemory`.
    ///
    /// # Errors
    /// Propagates RM's refusal.
    pub fn host_reserve_vidmem(&self, len: u64) -> Result<u32, RmError> {
        self.conn.reserve_gpga(len)
    }

    /// ★★★★★ **B1(d) PROBE — one FIXED `MapMemoryDma`, with RM's status and the VA it
    /// actually used.** `at` is binding, not a hint.
    ///
    /// # Errors
    /// Ioctl-level only — RM's refusal is [`MapOutcome::status`].
    pub fn host_map_dma_fixed(
        &self,
        h_dma: u32,
        h_memory: u32,
        len: u64,
    ...
    /// # Errors
    /// Propagates RM's refusal.
    pub fn host_unmap_dma(&self, h_dma: u32, gpu_va: u64) -> Result<(), RmError> {
