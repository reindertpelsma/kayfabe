...
//!
//! A replay answers with a [`kf_gsp::CommandPolicy`], and for a long time the only one
//! it could answer with was [`kf_gsp::EchoOk`] — the C's own acknowledge-everything
...
pub use ga10x::{CAPTURE_FB_SIZE_MB, gsp_model};
...
pub const FUNCTIONS: kf_gsp::FunctionCodes = kf_gsp::FunctionCodes {
...
/// full `major.minor.patch` through `kf_abi::versions`, never on a literal.
///
...
/// # Panics
///
/// If `kf_abi::versions::BENCH_DRIVER` has no wire table, or if the table describes
/// an element layout `kayfabe-gsp` rejects. Both are build-time facts about the version
/// tables, not run-time conditions.
#[must_use]
pub fn bench_abi() -> kf_gsp::GspAbi {
    let table = kf_abi::versions::table_for(kf_abi::versions::BENCH_DRIVER)
        .expect("the bench driver has a wire table");
    let wire = table.gsp_element_wire();
    let transport = match wire.transport() {
        None => kf_gsp::TransportHdr::None,
        Some(t) => kf_gsp::TransportHdr::Mctp {
    ...
    kf_gsp::GspAbi {
        ...
        element: kf_gsp::ElementLayout::new(
            wire.hdr_size(),
            wire.checksum_off(),
            wire.seqnum_off(),
            wire.elem_count_off(),
        ...
        init_args: kf_gsp::InitArgsLayout {
            shared_mem_pa_off: 0,
            pte_count_off: 8,
