...
//!   of [`kf_gsp::RegionMap`]), and the capture cannot answer.
//! - **GSP-D2** — the C never reads the guest's status-queue `readPtr`, because it has no
//!   flow control. Our [`kf_gsp::GspFsm::post`] reads it on every post.
//!
...
use kf_gsp::{GuestRam, RamRefused};
...
kf_util::assert_send_sync!(Answer, ReconKind, Reconstruction, Unobserved);
