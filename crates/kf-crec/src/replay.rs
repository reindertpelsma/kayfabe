//! Replay a recorded C capture against [`kf_gsp::GspFsm`], and project **both** sides
...
//!
//! Two diffs are therefore reported, both through [`kf_trace::diff`]:
//!
...
//!
//! Excluding a plane this crate does not implement is not sampling: [`kf_trace::diff`]
...
pub type PolicyFactory = fn() -> Box<dyn kf_gsp::CommandPolicy>;

/// The replay.
pub struct Replay<'a> {
    trace: &'a CTrace,
    abi: GspAbi,
    gsp: Box<dyn GspModel>,
...
impl<'a> Replay<'a> {
    ...
    ///
    /// Axis A is the caller's (`kf_abi::versions`), Axis B is the GA10x GSP model ([`crate::ga10x::gsp_model`]) — neither
    ...
    fn run_once(&self, lookahead: bool, recon: &[Reconstruction]) -> ReplayResult {
        ...
        while i < recs.len() {
            ...
            let reg = self.gsp.decode_reg(head.bar, head.a);
            ...
            if write {
                ...
                match fsm.mmio_read_with(self.gsp.as_ref(), head.bar, head.a) {
...
///
/// It exists so the one comparison in this harness is [`kf_trace::diff`] — there is
