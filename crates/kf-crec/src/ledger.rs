...
//! table ([`kf_gsp::LEDGER`]) and nothing else: it owns no policy of its own, and it
...
pub enum Verdict {
    /// A row of [`kf_gsp::LEDGER`] predicted it, and the C is the thing that is
    /// wrong.
...
pub fn classify(txn: &Txn, c: Option<&Note>, rust: Option<&Note>) -> Verdict {
    ...
    if matches!(txn.refusal, Some(kf_gsp::GspFault::QueueFull { .. })) {
    ...
    if txn.reg == Some(GspReg::GspFalconCpuctl)
        ...
        && txn
            .transitions
            .iter()
            .any(|t| matches!(t, kf_gsp::Transition::E2 | kf_gsp::Transition::E3))
        && rust.is_none()
        && matches!(
            c,
            Some(Note::Decoded(
...
/// Diff each transaction with [`kf_trace::diff`] and classify every divergence.
///
...
pub fn census(res: &ReplayResult) -> Census {
    ...
    for (n, t) in res.txns.iter().enumerate() {
        ...
        while base <= ce.len().max(re.len()) {
            let Some(d) =
                kf_trace::diff(ce.get(base..).unwrap_or(&[]), re.get(base..).unwrap_or(&[]))
            else {
                break;
            };
            let at = base + d.at;
...
/// Sanity: [`kf_trace::Divergence`] is what [`census`] is built on, so a change to
