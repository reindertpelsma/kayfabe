...
//! [`kf_gsp::Transition`] is recorded on every step so a test asserts the transition
...
impl<'a> RecordingRam<'a> {
    ...
    pub fn refused(&mut self, fault: &GspFault) {
        self.log.push(Observation::Refused {
            fault: fault_tag(&fault),
        });
    }
}

impl GuestRam for RecordingRam<'_> {
    fn read(&mut self, gpa: u64, buf: &mut [u8]) -> Result<(), kf_gsp::fault::RamRefused> {
        self.reads += 1;
        self.inner.read(gpa, buf)
    }

    fn write(&mut self, gpa: u64, bytes: &[u8]) -> Result<(), kf_gsp::fault::RamRefused> {
        self.writes += 1;
...
/// The tag for a GSP fault. ⊘ A free function, not `impl Faulted for GspFault`: neither the trait
/// nor the type lives in this oracle crate (the orphan rule), and the product must not carry it.
#[must_use]
pub fn fault_tag(f: &GspFault) -> FaultTag {
    GspFaultTag(f).tag()
}

struct GspFaultTag<'a>(&'a GspFault);
...
kf_util::assert_send_sync!(Observation, ObservationLog, Projection, Divergence, QueueId);
