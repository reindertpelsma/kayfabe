...
//! 2. it drives [`kf_gsp::EchoOk`], the C's own *acknowledge-everything* baseline, so
...
//! So four of the five [`kf_rm::inittables::InitTablePolicy`] replies had **no
...
//!
//! This file fixes both halves: `cap1b` for the reach, `kf_crec::served_policy` for
//! the plane.
//!
...
/// ★ The served chain the replay answers with — `kf_rm::served_policy` over the old tree's
/// captured GA106 rows (the TEST FIXTURE `kf-rm/tests/support/ga106.rs`, never a source).
///
/// Built here rather than in `kf_crec`'s lib because the GA106 facts are test-only: a lib
/// `served_policy()` would need a per-die row in `src/`, the shape v3 exists to end.
///
/// ⊘ Every latch fresh per run (a shared one would carry one round's registrations into the
/// next), the census never `set_probe_arm`ed (the C never probe-armed anything), and **no
/// object-model seat**: `cap1`/`cap1b` are the C's replies, and the C had no object model —
/// installing one would diff our object model's answers against the C's forged ones.
fn served_policy() -> Box<dyn kf_gsp::CommandPolicy> {
    kf_rm::served_policy(
        ga106::board(),
        ga106::host(),
        bench_abi().driver,
...
fn control_cmd(cmd: &kf_gsp::RpcCommand) -> Option<u32> {
...
fn the_continuation_elements_cap1_could_not_witness_are_witnessed_here() {
    ...
    let audit = |t: &CTrace| -> (usize, usize, usize) {
        ...
        // `kf_gsp`'s own decode checks. Present on a head, absent on a continuation.
...
fn cap1b_closes_the_replay_past_cap1s_wall_and_the_new_wall_is_a_different_finding() {
    ...
    assert!(r.max_lookahead <= kf_crec::oracle::LOOKAHEAD_LIMIT);
}
...
fn the_multi_element_command_cap1_died_on_is_served_here() {
    ...
    assert_eq!(
        control_cmd(cmd),
        Some(kf_abi::regaccessmap::NV2080_CTRL_CMD_INTERNAL_GPU_GET_USER_REGISTER_ACCESS_MAP),
...
fn every_control_this_port_serves_is_exercised_by_the_replay() {
    ...
    // `kf_abi::gvaspacepdes`. What is genuinely uncovered here is the ENVELOPE for this
    ...
    let outside_the_closure_limit: BTreeSet<WantedTable> = [
        ...
        // by an oracle a differential could not match: `kf_abi::cecaps`'s own tests
        ...
        //
        // ★ Its reply plane is covered where it can be: `kf_abi::fmbpromote`'s own
        ...
        //     `kf_abi::cudartinit`'s own tests, and the per-id BISECTION there is a
    ...
    // `kf_abi::gspfeatures`'s unit tests assert byte-for-byte, and the honest reading
    ...
    // `kf_rm::sticky`'s module docs `[measured]` that not one control word in the
...
fn every_control_the_oracle_asks_is_either_served_or_triaged() {
    ...
    // purpose; see `kf_rm::sweep::SweepDisposition` for the five outcomes and
    ...
    // captured row does not carry. See `kf_abi::fmbsize`.
...
fn the_served_replies_are_the_ones_posted_and_each_carries_the_result_it_earned() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            // out is `kf_abi::eventnotify`'s own round-trip tests, not this line;
            ...
            // what makes the four-zero body the right one is `kf_abi::l2evict`'s own
...
fn our_interrupt_kernel_table_is_byte_identical_to_a_real_ga106s_own_reply() {
    ...
    // `ga106::INTR_TABLE` (the old `ga10x::GA106_INTR_TABLE`) and the subtree map. The two agree on every
    ...
    let intr_txn = r
        ...
        .find(|(_, c)| {
            control_cmd(c)
                == Some(kf_abi::inittables::NV2080_CTRL_CMD_INTERNAL_INTR_GET_KERNEL_TABLE)
        })
        .map(|(t, _)| *t)
        .expect("the guest asks for the interrupt kernel table");
    // ★ 4e9aae9b: +2 — the recorded guest's BCR_CTRL reads are now decoded (58e03230).
