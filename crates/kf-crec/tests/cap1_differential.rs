...
fn every_gsp_register_read_within_the_oracles_reach_is_served_exactly_as_the_c_served_it() {
    ...
    let only_registers = |p: &kf_crec::Projected, before: bool| -> Vec<TraceEvent> {
        p.events
            .iter()
            .zip(&p.notes)
            .zip(&p.txn)
...
fn without_a_named_reconstruction_the_capture_stops_at_the_bind_and_gsp_d8_is_why() {
    ...
    assert_eq!(
        r.txns[txn].reg,
        Some(kf_arch::gsp::GspReg::GspFalconMailbox1),
    ...
    assert_eq!(
        r.txns[txn].refusal,
        Some(GspFault::GuestRam(kf_gsp::RamRefused {
            gpa: 0x1_2720_0000,
            len: 1032,
            why: kf_crec::oracle::UNOBSERVED,
        })),
...
fn exactly_two_reconstructions_carry_the_replay_past_the_bind_and_both_are_ledger_rows() {
    ...
    assert!(
        r.max_lookahead <= kf_crec::oracle::LOOKAHEAD_LIMIT,
