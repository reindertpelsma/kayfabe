...
//! [`Plane::trap_write`], which is [`kf_trap::trap::TrapPath`] and nothing else.

use kf_trap::bitmap::RungBitmap;
...
use kf_trap::ring::PrivRing;
use kf_trap::token::{Claim, Release, Route, TokenWord};
use kf_trap::trap::{Action, Class, TrapPath};
use kf_trap::wake::WakeWord;
use kf_trap::REACT_ROUNDS;
...
pub struct Plane<'v> {
    ...
    pub doorbell: kf_trap::trappolicy::DoorbellPlacement,
...
impl<'v> Plane<'v> {
    ...
    pub fn for_family(
        ...
        Plane {
            ...
            ring: PrivRing::new(),
            token_mask,
            timer: kf_trap::timer::timer_regs_for(family),
    ...
    /// ★ A HOST COMPLETION for token `tok`'s channel: ring its own token from inside the plane,
    /// exactly as a doorbell would (stamp = the token's own; there is no guest ring position behind
    ...
    pub fn trap_regions(&self) -> Vec<kf_trap::trappolicy::TrapRegion> {
        kf_trap::trappolicy::trap_regions(self.doorbell, self.bar0_bytes)
    }
    ...
    /// See `kf_trap::memmap` and `THE_CONSTRAINTS.md` §53.
    ...
    pub fn trap_write(&self, class: Class, bar: u8, off: u32, val: u64, width: u8) -> Action {
        if !kf_trap::trappolicy::may_trap_write(kf_trap::vmm::Bar(bar), off as u64, self.doorbell) {
            return Action::None;
        }
