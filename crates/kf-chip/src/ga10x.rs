//! GA10x — the family row ([`Arch`]) the emulated board classifies objects with.
//!
//! ⊘ **v3 (w826):** copied from the old `kayfabe-chips`. Its Ada/Hopper/Blackwell siblings were NOT
//! copied: they delegated every non-GSP answer to `MockArch`. In v3 an unbuilt family refuses by
//! name (`Family::arch` → `RowUnbuilt`). The history below is the old tree's.
//!
//! ## ★★★ Why this is not `Ad10xArch` with a different name
//!
//! The old tree's `Ad10xArch` and `Gh100Arch` were `MockArch` in every respect except
...
//! [`kf_arch::MethodState`] and [`Ga10xPushbuffer::decode_run`] accumulates
...
//! [`kf_arch::PushRange::va`] now says so in the type. `kayfabe_fwd::read_pushbuffer`
...
//! `kf_gsp::boot`, which this value never reaches). `None` is the honest answer:
//! *this `Arch` does not carry a GSP register model.*

use kf_abi::generated::classes as nv;
use kf_abi::submit;
use kf_arch::gsp::GspModel;
use kf_arch::ids::{ClassId, ControlCmd, EngineKind, GpuVa, Pdb, RunlistId, VChid};
use kf_arch::{
...
impl Arch for Ga10xArch {
    ...
    fn classify(&self, class: ClassId) -> ObjectKind {
        match class.0 {
            ...
            // `kf_abi::rc::EngineRoute::for_engine` (`rc.rs:115`) maps both to
...
/// [`kf_arch::PteDecode::Sparse`] distinguishable here; without it a declared-empty
...
///    [`kf_arch::PteDecode::Pde::edge`] (small) and `also` (big).
...
/// [`kf_arch::GmmuFmt::level_shift`]'s contract describes: "it may contain more rows
/// than [`kf_arch::GmmuFmt::levels`] counts a single walk to be deep: where a regime
/// offers two alternative leaf tables … those are two rows, reachable only via
/// [`kf_arch::PdeEdge::child_level`]". Rows 4 and 5 are alternatives, not successive
...
///
/// ★ **Exhaustive is the contract** ([`kf_arch::GmmuFmt::page_sizes`]) and it is what
...
/// and `0x8C`.
///
/// Every number is `kf_abi::submit`'s, per decision #2's quarantine; this type is
...
/// fact kinds.
///
/// Every bit position is `kf_abi::submit`'s, cited to `ogkm-580`, and none is
...
impl Ga10xPushbuffer {
    ...
    /// `kf_arch::METHOD_SLOTS` allows. Nothing outside this file may assume a slot
    /// means anything.
    ...
    fn ce_completion(
        ...
        Some(Some(kf_arch::CeCompletion {
            addr: GpuVa(
    ...
    ///   [`kf_arch::CeWork::Fill`] has no way to express it.
    ...
    /// - **An element size that does not divide 4.** [`kf_arch::CeWork::Fill`]
    ...
    /// A latched `SET_{SRC,DST}_PHYS_MODE` slot → [`kf_arch::PhysTarget`]. An operand
    /// that never wrote the register is `LocalFb`, the engine's own reset value
    /// (`ogkm-580: clc7b5.h:68`) — faithful, not a guess.
    fn phys_target(latched: Option<u32>) -> kf_arch::PhysTarget {
        use kf_arch::PhysTarget;
    ...
    ///   fill lands at `dst + i·element` — while [`kf_arch::CeWork::Fill`]'s
    ...
    ///   see.
    ///
    /// ⊘ And [`kf_arch::CeWork::Scrub`] is **never** produced: `MEMORY_SCRUB_ENABLE`
...
impl PushbufferAbi for Ga10xPushbuffer {
    ...
    /// [`kf_abi::submit::gp_entry_decode`] names its own field `gpu_va`.
    ///
        ...
        run.iter()
            .map(|(header, args)| {
                ...
                for (i, &arg) in args.iter().enumerate() {
                    ...
                    // of every UVM CE push. See `kf_abi::submit::uvm_sw`.
